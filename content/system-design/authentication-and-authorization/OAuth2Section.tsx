...
export function OAuth2Section() {
  return (
    <section>
      <h2 id="oauth2" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">OAuth 2.0 grant flows</h3>

      <div className="space-y-4 mb-8">
        {OAUTH_FLOWS.map(({ flow, actors, use, steps, why }) => (
          <div key={flow} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <p className="text-[11px] font-semibold text-brand-text">{flow}</p>
              <p className="text-[9px] text-brand-text-muted">Use: {use}</p>
            </div>
            <div className="px-4 py-3 space-y-3">
              <div>
                <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-1.5">Actors: {actors}</p>
                <ol className="space-y-1">
                  {steps.map((step, i) => (
                    <li key={i} className="flex gap-2 text-[11px]">
                      <span className="w-4 h-4 rounded-full bg-brand-primary/10 text-brand-primary text-[9px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                      <span className="text-brand-text-muted">{step}</span>
                    </li>
                  ))}
                </ol>
              </div>
              <div className="border-l-2 border-brand-primary/30 pl-3 text-[11px] text-brand-text-muted">
                <span className="font-medium text-brand-text/70">Why this flow: </span>
                {why}
              </div>
            </div>
          </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Scopes: the unit of delegation</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Scope</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Permits</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {SCOPES_EXAMPLE.map(({ scope, meaning }) => (
              <tr key={scope}>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-primary align-top">{scope}</td>
                <td className="py-2 text-brand-text-muted">{meaning}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">OIDC: identity on top of OAuth 2.0</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose space-y-3">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">What OIDC adds to OAuth 2.0</p>
        {[
          ...
          <div key={term} className="flex gap-3 p-2.5 bg-brand-surface/30 rounded-xl text-[11px]">
            <span className="font-mono font-semibold text-brand-primary w-36 shrink-0">{term}</span>
            <span className="text-brand-text-muted">{def}</span>
          </div>
        ))}
      </div>
