...
can_edit = "write:posts" in roles[user.role]  # True`,
  ...
  },
  {
    ...
    color: "text-brand-chart-4 bg-brand-chart-4/10 border-brand-chart-4/20",
...
def can_edit(user, post, context):
  ...
  },
  {
    ...
    color: "text-brand-chart-3 bg-brand-chart-3/10 border-brand-chart-3/20",
...
export function AuthZModelsSection() {
  return (
    <section>
      <h2 id="authz-models" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="space-y-6 mb-8">
        {MODELS.map(({ name, full, color, model, example, strengths, weaknesses, when }) => (
          <div key={name} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-brand-border bg-brand-surface/30">
              <span className={`text-[9px] font-mono font-semibold px-2 py-0.5 rounded border ${color}`}>{name}</span>
              <span className="text-[12px] font-semibold text-brand-text">{full}</span>
            </div>
            <div className="p-4 space-y-4">
              <p className="text-[11px] text-brand-text-muted">{model}</p>
              <div>
                <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-2">Example</p>
                <pre className="text-[9px] font-mono bg-brand-surface/50 rounded-xl px-4 py-3 overflow-x-auto leading-relaxed text-brand-text/80">
                  {example}
                </pre>
              </div>
              <div className="grid sm:grid-cols-2 gap-3 text-[11px]">
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-brand-primary mb-1.5">Strengths</p>
                  <ul className="space-y-1">
                    {strengths.map((s) => (
                      <li key={s} className="flex gap-1.5 text-brand-text-muted">
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-brand-warning mb-1.5">Weaknesses</p>
                  <ul className="space-y-1">
                    {weaknesses.map((w) => (
                      <li key={w} className="flex gap-1.5 text-brand-text-muted">
                        <span className="text-brand-warning shrink-0">-</span> {w}
                      </li>
                    ))}
                  </ul>
                </div>
