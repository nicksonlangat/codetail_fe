...
export function SessionsAndTokensSection() {
  return (
    <section>
      <h2 id="sessions-tokens" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Sessions vs Tokens
      </h2>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Server-side sessions</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Session flow</p>
...
3. POST /auth/logout
   ...
   -> User instantly logged out, next request finds no session`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">JWTs (JSON Web Tokens)</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-2 mb-3">Access tokens and refresh tokens</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Refresh token flow</p>
        <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-4 py-3 overflow-x-auto leading-relaxed">
...
4. Logout:
   ...
   -> User cannot refresh -> effectively logged out`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Sessions vs JWTs: when to use each</h3>

      <div className="overflow-x-auto mb-6">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Property</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Session</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">JWT</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {COMPARISON_ROWS.map(({ property, session, jwt }) => (
              <tr key={property}>
                <td className="py-2.5 pr-4 font-medium text-brand-text/80 align-top">{property}</td>
                <td className="py-2.5 pr-4 text-brand-text-muted align-top">{session}</td>
                <td className="py-2.5 text-brand-text-muted align-top">{jwt}</td>
              </tr>
            ))}
          </tbody>
        </table>
