...
export function AuthNvsAuthZSection() {
  return (
    <section>
      <h2 id="authn-authz" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-8 not-prose">
        <div className="grid sm:grid-cols-2 gap-4 text-[11px]">
          <div className="p-3 bg-brand-surface/40 rounded-xl space-y-2">
            <p className="font-semibold text-brand-text text-[12px]">Authentication (AuthN)</p>
            <p className="text-brand-text-muted">Who are you? Prove your identity.</p>
            <div className="space-y-1">
              {["Password + username", "OAuth token from a trusted provider", "mTLS client certificate", "SSH key", "Passkey / WebAuthn"].map((m) => (
                <p key={m} className="flex items-center gap-1.5 text-brand-text-muted">
                  <span className="text-brand-primary">→</span> {m}
                </p>
              ))}
            </div>
            <p className="text-[10px] text-brand-text-muted">Result: verified identity (e.g., user_123)</p>
          </div>
          <div className="p-3 bg-brand-surface/40 rounded-xl space-y-2">
            <p className="font-semibold text-brand-text text-[12px]">Authorization (AuthZ)</p>
            <p className="text-brand-text-muted">What can you do? Enforce access policy.</p>
            <div className="space-y-1">
            ...
            <p className="text-[10px] text-brand-text-muted">Result: allow or deny</p>
          </div>
        </div>
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The sequence in practice</h3>

      <div className="space-y-2 mb-8">
        {EXAMPLES.map(({ scenario, authn, authz }) => (
          <div key={scenario} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2 border-b border-brand-border bg-brand-surface/30">
              <p className="text-[10px] text-brand-text-muted">{scenario}</p>
            </div>
            <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-brand-border/50">
              <div className="px-4 py-2.5">
                <p className="text-[9px] uppercase tracking-wider text-brand-chart-2 mb-1">AuthN result</p>
                <p className="text-[11px] text-brand-text-muted">{authn}</p>
              </div>
              <div className="px-4 py-2.5">
                <p className="text-[9px] uppercase tracking-wider text-brand-primary mb-1">AuthZ check</p>
                <p className="text-[11px] text-brand-text-muted">{authz}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Common mistakes</h3>
        ...
        {COMMON_MISTAKES.map(({ mistake, detail, severity }) => (
          <div
            ...
            className={`flex gap-3 p-3 rounded-xl border ${
              severity === "critical" ? "border-brand-destructive/20 bg-brand-destructive/5" : "border-brand-warning/20 bg-brand-warning/5"
            }`}
          >
            <span className={`font-bold text-sm shrink-0 mt-0.5 ${severity === "critical" ? "text-brand-destructive" : "text-brand-warning"}`}>!</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{mistake}</p>
