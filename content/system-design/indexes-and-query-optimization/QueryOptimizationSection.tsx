...
export function QueryOptimizationSection() {
  return (
    <section>
      <h2 id="query-optimization" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Reading EXPLAIN output</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-8 not-prose">
        ...
        <pre className="text-[10px] font-mono text-brand-text/80 leading-relaxed whitespace-pre overflow-x-auto">{`EXPLAIN ANALYZE
...
Execution Time: 0.05 ms            <- 2,500x faster`}</pre>
        <div className="mt-3 grid sm:grid-cols-3 gap-2 text-[10px]">
          {[
            ...
            <div key={key} className="p-2 rounded-lg bg-brand-surface">
              <span className="font-mono font-semibold text-brand-primary">{key}</span>
              <p className="text-brand-text-muted mt-0.5">{meaning}</p>
            </div>
          ))}
        </div>
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Common slow query patterns</h3>
          ...
          <div key={name} className="rounded-xl border border-brand-border bg-white overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-brand-destructive/70 font-medium mb-1.5">Problematic</p>
                  <pre className="text-[9px] font-mono bg-brand-surface rounded-lg px-3 py-2.5 overflow-x-auto whitespace-pre leading-relaxed text-brand-text/70">
                    {bad}
                  </pre>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-brand-primary/70 font-medium mb-1.5">Fixed</p>
                  <pre className="text-[9px] font-mono bg-brand-surface rounded-lg px-3 py-2.5 overflow-x-auto whitespace-pre leading-relaxed text-brand-text/70">
                    {good}
                  </pre>
                </div>
              </div>
              <p className="text-[10px] text-brand-text-subtle italic border-t border-brand-border/50 pt-2">{note}</p>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">When not to add an index</h3>
