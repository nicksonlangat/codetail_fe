...
ORDER BY email                       -- sort`,
  ...
  },
  {
    ...
    tagColor: "bg-brand-chart-4/10 text-brand-chart-4",
...
WHERE age > 30              -- skips two leftmost columns`,
  ...
  },
  {
    name: "Covering index",
    tag: "index-only scan",
    tagColor: "bg-brand-chart-2/10 text-brand-chart-2",
...
-- Result: index-only scan, zero heap fetches`,
  ...
  },
  {
    ...
    tagColor: "bg-brand-chart-3/10 text-brand-chart-3",
...
export function IndexTypesSection() {
  return (
    <section>
      <h2 id="index-types" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        ...
        {INDEX_TYPES.map(({ name, tag, tagColor, desc, code, note }) => (
          <div key={name} className="rounded-xl border border-brand-border bg-white overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              ...
              <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-3 py-3 overflow-x-auto whitespace-pre leading-relaxed">
                {code}
              </pre>
              <div className="flex gap-2 items-start p-2.5 rounded-lg bg-brand-surface/50">
                <span className="text-[10px] font-bold text-brand-text-muted shrink-0 mt-0.5">Note</span>
                <p className="text-[10px] text-brand-text-muted leading-relaxed">{note}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Composite index column order</h3>
        ...
        <div className="space-y-1.5">
          {[
            ...
            <div key={query} className="flex items-start gap-3 text-[10px]">
              <span
                className={`shrink-0 font-mono font-bold w-14 text-right ${
                  uses === "Yes" ? "text-brand-primary" : uses === "No" ? "text-brand-destructive" : "text-brand-warning"
                }`}
              >
                {uses}
              </span>
              <code className="font-mono text-brand-text/70 flex-1">{query}</code>
              <span className="text-brand-text-subtle shrink-0 hidden sm:block">{note}</span>
            </div>
          ))}
        </div>
      </div>
