export function AdvancedSection() {
  return (
    <section>
      <h2 id="advanced-features" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        SSL Termination, Sticky Sessions, and Global Load Balancing
      </h2>

      <h3 className="text-base font-semibold text-brand-text mt-2 mb-3">SSL/TLS termination</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-5 mb-6 not-prose">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-1 text-[11px]">
          {[
            ...
            "arrow" in item ? (
              <div key={i} className="flex items-center gap-1 text-[10px] text-brand-text-muted">
              ...
              <div
                ...
                className={`flex flex-col items-center p-3 rounded-xl border text-center ${
                  "highlight" in item && item.highlight ? "border-brand-primary/30 bg-brand-primary/5" : "border-brand-border bg-white"
                }`}
              >
                <span className="text-lg mb-1">{item.icon}</span>
                <span className="font-semibold text-brand-text">{item.label}</span>
                <span className="text-[9px] text-brand-text-muted">{item.sub}</span>
              </div>
            )
          )}
        </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Sticky sessions (session affinity)</h3>
      ...
      <div className="space-y-2 mb-6">
        {[
          ...
          <div key={method} className="p-4 rounded-xl border border-brand-border bg-white text-[12px]">
            <p className="font-semibold text-brand-text mb-1">{method}</p>
            <p className="text-brand-text-muted mb-2 leading-relaxed">{how}</p>
            <p className="text-[11px] text-brand-text-subtle italic">{note}</p>
          </div>
        ))}
      </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Global load balancing</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-6">
        <div className="p-4 bg-white border border-brand-border rounded-xl space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-base">🌍</span>
            <span className="text-[13px] font-semibold text-brand-text">GeoDNS</span>
          </div>
          ...
          <div className="text-[10px] space-y-1 text-brand-text-muted">
            <p>
              <span className="text-brand-text font-medium">Latency reduction:</span> 50-200ms for global users
            </p>
            <p>
              <span className="text-brand-text font-medium">Propagation delay:</span> Bound by DNS TTL
            </p>
            <p>
              <span className="text-brand-text font-medium">Used by:</span> Cloudflare, AWS Route 53, Google Cloud DNS
            </p>
          </div>
        </div>
        <div className="p-4 bg-white border border-brand-border rounded-xl space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-base">📡</span>
            <span className="text-[13px] font-semibold text-brand-text">Anycast</span>
          </div>
          <p className="text-[11px] text-brand-text-muted leading-relaxed">
          ...
          <div className="text-[10px] space-y-1 text-brand-text-muted">
            <p>
              <span className="text-brand-text font-medium">Failover speed:</span> Sub-second (BGP convergence)
            </p>
            <p>
              <span className="text-brand-text font-medium">DDoS resistance:</span> Attack traffic absorbed across PoPs
            </p>
            <p>
              <span className="text-brand-text font-medium">Used by:</span> Cloudflare, Fastly, all major CDNs
            </p>
          </div>
        </div>
      </div>
