...
export function MicroservicesSection() {
  return (
    <section>
      <h2 id="microservices" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-4 mb-3">Bounded contexts: where to draw the lines</h3>
      ...
      <div className="space-y-2 mb-8">
        {BOUNDED_CONTEXTS.map(({ context, owns, publishedEvents, dependencies }) => (
          <div key={context} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-primary">{context} Service</span>
            </div>
            <div className="grid sm:grid-cols-3 gap-x-4 gap-y-1 px-4 py-3 text-[11px]">
              <p>
                <span className="font-medium text-brand-text/80">Owns:</span> <span className="text-brand-text-muted">{owns}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Emits:</span>{" "}
                <span className="text-brand-text-muted font-mono text-[10px]">{publishedEvents}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Depends on:</span> <span className="text-brand-text-muted">{dependencies}</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The distributed systems tax</h3>
      ...
      <div className="space-y-2">
        {DISTRIBUTED_TAX.map(({ cost, detail, severity }) => (
          <div
            key={cost}
            className={`flex gap-3 p-3 rounded-xl border ${severity === "high" ? "border-brand-warning/20 bg-brand-warning/5" : "border-brand-border bg-white"}`}
          >
            <span className={`text-sm shrink-0 mt-0.5 font-bold ${severity === "high" ? "text-brand-warning" : "text-brand-text-muted"}`}>
              {severity === "high" ? "!" : "~"}
            </span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{cost}</p>
