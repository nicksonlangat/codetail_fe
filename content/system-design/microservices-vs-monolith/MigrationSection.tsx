...
export function MigrationSection() {
  return (
    <section>
      <h2 id="migration" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="space-y-2 mb-8">
        {STRANGLER_STEPS.map(({ step, name, detail }) => (
          <div key={step} className="flex gap-4 p-3 bg-white border border-brand-border rounded-xl">
            <span className="w-6 h-6 rounded-full bg-brand-primary/10 text-brand-primary text-[10px] font-bold font-mono flex items-center justify-center shrink-0 mt-0.5">
              {step}
            </span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{name}</p>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The hardest part: splitting the database</h3>
      ...
      <div className="space-y-3 mb-6">
        {DATA_DECOMPOSITION_PROBLEMS.map(({ problem, symptom, approach }) => (
          <div key={problem} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{problem}</span>
            </div>
            <div className="px-4 py-3 space-y-1.5 text-[11px]">
              <p>
                <span className="font-medium text-brand-warning">Symptom: </span>
                <span className="text-brand-text-muted">{symptom}</span>
              </p>
              <p>
                <span className="font-medium text-brand-primary">Approach: </span>
                <span className="text-brand-text-muted">{approach}</span>
              </p>
            </div>
          </div>
        ))}
