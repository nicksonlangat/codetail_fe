...
export function WhatToServeSection() {
  return (
    <section>
      <h2 id="what-to-serve" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-8 not-prose">
        <div className="bg-white border border-brand-border rounded-xl overflow-hidden">
          <div className="px-4 py-2.5 border-b border-brand-border bg-brand-primary/5">
            <p className="text-[10px] font-semibold text-brand-primary">Cache on CDN</p>
          </div>
          <div className="divide-y divide-brand-border/50">
            {CDN_YES.map(({ item, why }) => (
              <div key={item} className="px-4 py-2.5">
                <p className="text-[11px] font-medium text-brand-text mb-0.5">{item}</p>
                <p className="text-[10px] text-brand-text-muted">{why}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white border border-brand-border rounded-xl overflow-hidden">
          <div className="px-4 py-2.5 border-b border-brand-border bg-brand-destructive/5">
            <p className="text-[10px] font-semibold text-brand-destructive">Do NOT cache on CDN</p>
          </div>
          <div className="divide-y divide-brand-border/50">
            {CDN_NO.map(({ item, why }) => (
              <div key={item} className="px-4 py-2.5">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Security headers via CDN</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              ...
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Example value</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Purpose</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {SECURITY_HEADERS.map(({ header, value, desc }) => (
              <tr key={header}>
                ...
                <td className="py-2 pr-4 font-mono text-[9px] text-brand-text-muted align-top">{value}</td>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The Vary header</h3>
