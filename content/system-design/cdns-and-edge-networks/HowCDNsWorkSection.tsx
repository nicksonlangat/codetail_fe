...
export function HowCDNsWorkSection() {
  return (
    <section>
      <h2 id="how-cdns-work" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        How CDNs Work
      </h2>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-4 mb-3">Anycast routing</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Pull vs push CDN</h3>

      <div className="grid sm:grid-cols-2 gap-3 mb-8 not-prose">
        <div className="bg-white border border-brand-border rounded-xl p-4">
          <p className="text-[11px] font-semibold text-brand-text mb-2">Pull CDN (most common)</p>
          ...
          <div className="space-y-1 text-[10px]">
            {["Zero setup for new assets", "Only caches what is actually requested", "First request per PoP is a cache miss", "Used by: Cloudflare, CloudFront, Fastly"].map((p) => (
              <p key={p} className="flex gap-1.5 text-brand-text-muted">
                <span className="text-brand-primary">→</span> {p}
              </p>
            ))}
          </div>
        </div>
        <div className="bg-white border border-brand-border rounded-xl p-4">
          <p className="text-[11px] font-semibold text-brand-text mb-2">Push CDN</p>
          <p className="text-[11px] text-brand-text-muted mb-3">
          ...
          <div className="space-y-1 text-[10px]">
            {["Zero cache misses after push", "Requires explicit upload on every deploy", "Storage cost at every PoP", "Used by: Rackspace Cloud Files, Bunny CDN"].map((p) => (
              <p key={p} className="flex gap-1.5 text-brand-text-muted">
                <span className="text-brand-primary">→</span> {p}
              </p>
            ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Major providers</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              {["Provider", "PoPs", "Coverage", "Notes"].map((h) => (
                <th key={h} className="text-left py-2 pr-4 text-brand-text-muted font-medium">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-brand-border/50">
            {POP_TABLE.map(({ provider, pops, locations, note }) => (
              <tr key={provider}>
                <td className="py-2.5 pr-4 font-semibold text-brand-text/80 align-top">{provider}</td>
                <td className="py-2.5 pr-4 font-mono text-brand-primary align-top">{pops}</td>
                <td className="py-2.5 pr-4 text-brand-text-muted align-top">{locations}</td>
