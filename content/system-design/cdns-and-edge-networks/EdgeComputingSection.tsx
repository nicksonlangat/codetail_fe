...
export function EdgeComputingSection() {
  return (
    <section>
      <h2 id="edge-computing" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What to do at the edge</h3>

      <div className="space-y-2 mb-8">
        {EDGE_USE_CASES.map(({ useCase, detail, platforms }) => (
          <div key={useCase} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{useCase}</span>
            </div>
            <div className="px-4 py-3 space-y-1.5">
              <p className="text-[11px] text-brand-text-muted">{detail}</p>
              <p className="text-[10px]">
                <span className="font-medium text-brand-text-muted">Platforms: </span>
                <span className="font-mono text-brand-text-subtle">{platforms}</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Edge runtime constraints</h3>

      <div className="space-y-2 mb-6">
        {EDGE_CONSTRAINTS.map(({ constraint, detail }) => (
          <div key={constraint} className="flex gap-3 p-3 rounded-xl border border-brand-warning/20 bg-brand-warning/5">
            <span className="text-brand-warning text-sm shrink-0 mt-0.5 font-bold">!</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{constraint}</p>
              <p className="text-[11px] text-brand-text-muted">{detail}</p>
            </div>
          </div>
        ))}
