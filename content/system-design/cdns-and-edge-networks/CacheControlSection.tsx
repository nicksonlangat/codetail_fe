...
export function CacheControlSection() {
  return (
    <section>
      <h2 id="cache-control" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Key directives</h3>

      <div className="space-y-2 mb-8">
        {DIRECTIVES.map(({ directive, scope, meaning, example }) => (
          <div key={directive} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2 border-b border-brand-border bg-brand-surface/30">
              <span className="font-mono text-[10px] font-semibold text-brand-primary">{directive}</span>
              <span
                className={`ml-auto text-[8px] font-mono px-1.5 py-0.5 rounded border ${
                  scope === "CDN only"
                    ...
                    : scope === "Browser"
                    ? "text-brand-chart-2 bg-brand-chart-2/10 border-brand-chart-2/20"
            ...
            <div className="px-4 py-2.5 space-y-1">
              <p className="text-[11px] text-brand-text-muted">{meaning}</p>
              <pre className="text-[9px] font-mono text-brand-text-subtle">{example}</pre>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Caching strategies by asset type</h3>

      <div className="space-y-2 mb-8">
        {ASSET_STRATEGIES.map(({ asset, strategy, why }) => (
          <div key={asset} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <p className="text-[10px] font-medium text-brand-text/80">{asset}</p>
            </div>
            <div className="px-4 py-3 space-y-1.5">
              <pre className="text-[9px] font-mono text-brand-primary bg-brand-primary/5 border border-brand-primary/10 rounded-lg px-3 py-1.5 overflow-x-auto">
                {strategy}
              </pre>
              <p className="text-[11px] text-brand-text-muted">{why}</p>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Cache invalidation</h3>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={strategy} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{strategy}</p>
