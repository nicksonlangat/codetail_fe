...
export function ChoosingSection() {
  return (
    <section>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              ...
              <th className="text-left py-2 pr-2 text-brand-text-muted font-medium">RabbitMQ</th>
              <th className="text-left py-2 pr-2 text-brand-text-muted font-medium">SQS</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Kafka</th>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Operational cost</h3>

      <div className="space-y-3 mb-8">
        {OPERATIONAL_COSTS.map(({ system, setup, monitoring, failure, scaling }) => (
          <div key={system} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <p className="text-[12px] font-semibold text-brand-text">{system}</p>
            </div>
            <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2 px-4 py-3 text-[11px]">
              <p>
                <span className="font-medium text-brand-text/80">Setup:</span> <span className="text-brand-text-muted">{setup}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Scaling:</span> <span className="text-brand-text-muted">{scaling}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Monitor:</span> <span className="text-brand-text-muted">{monitoring}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Failure:</span> <span className="text-brand-text-muted">{failure}</span>
              </p>
            </div>
          </div>
        ))}
