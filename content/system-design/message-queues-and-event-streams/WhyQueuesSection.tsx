...
export function WhyQueuesSection() {
  return (
    <section>
      <h2 id="why-queues" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Synchronous vs async: a concrete example</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Scenario</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Model</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Latency</th>
          ...
          <tbody className="divide-y divide-brand-border/50">
            {LATENCY_COMPARISON.map(({ scenario, type, latency, risk }) => (
              <tr key={scenario}>
                <td className="py-2.5 pr-4 text-brand-text/80 align-top">{scenario}</td>
                <td className="py-2.5 pr-4 align-top">
                  <span
                    className={`text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded border ${
                      type === "async" ? "text-brand-primary bg-brand-primary/10 border-brand-primary/20" : "text-brand-warning bg-brand-warning/10 border-brand-warning/20"
                    }`}
                  >
                    {type}
                  </span>
                ...
                <td className="py-2.5 text-brand-text-muted align-top">{risk}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What a queue buys you</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Backpressure</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 not-prose space-y-3 mb-6">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">Backpressure strategies</p>
        {[
          {
          ...
          <div key={name} className="flex gap-3 p-3 bg-brand-surface/30 rounded-xl">
            <span className="text-brand-primary font-mono text-[10px] font-bold shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[11px] font-semibold text-brand-text mb-0.5">{name}</p>
              <p className="text-[10px] text-brand-text-muted">{desc}</p>
            </div>
          </div>
        ))}
      </div>
