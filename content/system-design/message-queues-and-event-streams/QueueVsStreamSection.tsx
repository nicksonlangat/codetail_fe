...
export function QueueVsStreamSection() {
  return (
    <section>
      <h2 id="queue-vs-stream" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-4 mb-3">Property comparison</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              ...
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Queue</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Stream</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {COMPARISON_ROWS.map(({ property, queue, stream }) => (
              <tr key={property}>
                ...
                <td className="py-2.5 pr-4 text-brand-text-muted align-top">{queue}</td>
                <td className="py-2.5 text-brand-text-muted align-top">{stream}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Real systems</h3>
        ...
        {SYSTEMS.map(({ name, type, protocol, best, notes }) => (
          <div key={name} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              ...
              <span className="ml-auto text-[9px] font-mono text-brand-text-muted">{protocol}</span>
            </div>
            <div className="px-4 py-3 space-y-1">
              <p className="text-[11px] text-brand-text/80">
                <span className="font-medium">Best for:</span> {best}
              </p>
              <p className="text-[11px] text-brand-text-muted">{notes}</p>
            </div>
          </div>
        ))}
      </div>
