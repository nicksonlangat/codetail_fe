const DELIVERY_GUARANTEES = [
  ...
  },
  {
    ...
    color: "text-brand-text-muted bg-brand-surface border-brand-border",
...
export function PatternsSection() {
  return (
    <section>
      <h2 id="patterns" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Delivery Guarantees and Messaging Patterns
      </h2>

      <h3 className="text-base font-semibold text-brand-text mt-6 mb-3">Delivery guarantees</h3>
      ...
      <div className="space-y-3 mb-8">
        {DELIVERY_GUARANTEES.map(({ name, aka, color, behavior, implementation, cost, when }) => (
          <div key={name} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className={`text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded border ${color}`}>{name}</span>
              <span className="text-[9px] text-brand-text-muted">{aka}</span>
            </div>
            <div className="px-4 py-3 space-y-2 text-[11px]">
              <p>
                <span className="font-medium text-brand-text/80">Behavior:</span> <span className="text-brand-text-muted">{behavior}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">How:</span> <span className="text-brand-text-muted">{implementation}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Cost:</span> <span className="text-brand-text-muted">{cost}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Use when:</span> <span className="text-brand-text-muted">{when}</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Messaging patterns</h3>

      <div className="space-y-4">
        {PATTERNS.map(({ name, type, diagram, desc, use, watch }) => (
          <div key={name} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[12px] font-semibold text-brand-text">{name}</span>
              <span
                className={`text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded border ${
                  type === "Queue"
                    ...
                    : "text-brand-text-muted bg-brand-surface border-brand-border"
                }`}
              >
                {type}
              </span>
            ...
            <div className="px-4 py-3 space-y-2">
              <pre className="text-[9px] font-mono bg-brand-surface/40 rounded-lg px-3 py-2 overflow-x-auto text-brand-text-muted leading-relaxed whitespace-pre">
                {diagram}
              </pre>
              <p className="text-[11px] text-brand-text-muted">{desc}</p>
              <p className="text-[11px]">
                <span className="font-medium text-brand-text/80">Use for:</span>{" "}
                <span className="text-brand-text-muted">{use}</span>
              </p>
              <p className="text-[11px]">
                <span className="font-medium text-brand-warning">Watch out:</span>{" "}
                <span className="text-brand-text-muted">{watch}</span>
              </p>
            </div>
          </div>
        ))}
