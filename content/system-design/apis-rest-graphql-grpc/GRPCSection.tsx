...
export function GRPCSection() {
  return (
    <section>
      <h2 id="grpc" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Protocol Buffers: how binary encoding works</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-4 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">
          From .proto definition to binary wire format
        </p>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <p className="text-[9px] text-brand-text-muted mb-2">Schema (.proto)</p>
            <pre className="text-[10px] font-mono bg-brand-surface rounded-lg px-3 py-2 leading-relaxed overflow-x-auto">
{`message User {
  int64  id         = 1;
  string name       = 2;
  string email      = 3;
...
// email: "a@ex.com", is_active: true`}
          ...
          <div>
            <p className="text-[9px] text-brand-text-muted mb-2">Wire format (binary, 24 bytes)</p>
            <pre className="text-[10px] font-mono bg-brand-surface rounded-lg px-3 py-2 leading-relaxed overflow-x-auto">
...
"is_active":true}`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Scalar types and wire sizes</h3>

      <div className="overflow-x-auto mb-8">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Type</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Wire type</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Bytes</th>
          ...
          <tbody className="divide-y divide-brand-border/50">
            {PROTOBUF_TYPES.map(({ type, wire, bytes, note }) => (
              <tr key={type}>
                <td className="py-2 pr-4 font-mono text-brand-primary text-[10px] align-top">{type}</td>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-text-muted align-top">{wire}</td>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-text/80 align-top whitespace-nowrap">{bytes}</td>
                <td className="py-2 text-brand-text-muted align-top">{note}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">HTTP/2 and multiplexing</h3>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mb-8 not-prose">
        {[
          ...
          <div key={label} className="bg-white border border-brand-border rounded-xl p-3">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Four streaming modes</h3>

      <div className="space-y-3 mb-8">
        {STREAMING_MODES.map(({ mode, pattern, proto, analogy, use }) => (
          <div key={mode} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{mode}</span>
              <span className="font-mono text-[9px] text-brand-text-muted">{pattern}</span>
              <span className="ml-auto text-[9px] text-brand-text-muted">like: {analogy}</span>
            </div>
            <div className="px-4 py-3 space-y-2">
              <pre className="text-[10px] font-mono bg-brand-surface/50 rounded-lg px-3 py-2 overflow-x-auto">{proto}</pre>
              <p className="text-[11px] text-brand-text-muted">{use}</p>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">When NOT to use gRPC</h3>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={reason} className="flex gap-3 p-3 rounded-xl border border-brand-warning/20 bg-brand-warning/5">
            <span className="text-brand-warning text-sm shrink-0 mt-0.5">!</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{reason}</p>
