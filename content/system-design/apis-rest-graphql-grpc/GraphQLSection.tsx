...
export function GraphQLSection() {
  return (
    <section>
      <h2 id="graphql" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Three operation types</h3>

      <div className="space-y-3 mb-8">
        {OPERATION_TYPES.map(({ type, direction, analogy, example, desc }) => (
          <div key={type} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-primary font-mono">{type}</span>
              <span className="text-[9px] text-brand-text-muted bg-brand-surface px-1.5 py-0.5 rounded">{direction}</span>
              <span className="text-[9px] text-brand-text-muted ml-auto">REST analogy: {analogy}</span>
            </div>
            <div className="px-4 py-3 space-y-2">
              <pre className="text-[10px] font-mono bg-brand-surface/50 rounded-lg px-3 py-2 overflow-x-auto">{example}</pre>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Schema design: a minimal example</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Schema definition language (SDL)</p>
        <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-4 py-3 overflow-x-auto leading-relaxed">
{`type Query {
  post(id: ID!): Post
...
type PageInfo { hasNextPage: Boolean!; endCursor: String }`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The DataLoader pattern</h3>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-6 not-prose">
        <div className="bg-white border border-brand-destructive/20 rounded-xl p-4">
          <p className="text-[9px] uppercase tracking-wider text-brand-destructive mb-2">Without DataLoader</p>
          <pre className="text-[10px] font-mono bg-brand-surface rounded-lg px-3 py-2 overflow-x-auto leading-relaxed text-brand-destructive/80">
{`// posts resolver
posts.forEach(post => {
  // N separate queries
  db.query(
...
// 100 posts = 101 queries`}
        ...
        <div className="bg-white border border-brand-primary/20 rounded-xl p-4">
          <p className="text-[9px] uppercase tracking-wider text-brand-primary mb-2">With DataLoader</p>
          <pre className="text-[10px] font-mono bg-brand-surface rounded-lg px-3 py-2 overflow-x-auto leading-relaxed text-brand-primary/80">
...
// 100 posts = 2 queries total`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Benefits and costs</h3>
        ...
        {TRADEOFFS.map(({ label, desc, type }) => (
          <div
            ...
            className={`flex gap-3 p-3 rounded-xl border ${
              type === "benefit" ? "border-brand-primary/20 bg-brand-primary/5" : type === "cost" ? "border-brand-warning/20 bg-brand-warning/5" : "border-brand-border bg-white"
            }`}
          >
            <span className={`text-sm shrink-0 mt-0.5 ${type === "benefit" ? "text-brand-primary" : type === "cost" ? "text-brand-warning" : "text-brand-text-muted"}`}>
