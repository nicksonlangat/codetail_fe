...
const STATUS_GROUPS = [
  ...
  },
  {
    range: "5xx",
    label: "Server error",
    color: "text-brand-destructive bg-brand-destructive/10 border-brand-destructive/20",
    codes: [
...
export function RESTSection() {
  return (
    <section>
      <h2 id="rest" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">HTTP verbs and their contracts</h3>

      <div className="overflow-x-auto mb-6">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Method</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Safe</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Idempotent</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Body</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Use</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {HTTP_METHODS.map(({ method, safe, idempotent, body, use }) => (
              <tr key={method}>
                <td className="py-2 pr-4 font-mono font-semibold text-brand-primary text-[11px]">{method}</td>
                <td className="py-2 pr-4">
                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${safe ? "text-brand-primary bg-brand-primary/10" : "text-brand-text-muted bg-brand-surface"}`}>
                    {safe ? "yes" : "no"}
                  </span>
                </td>
                <td className="py-2 pr-4">
                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${idempotent ? "text-brand-primary bg-brand-primary/10" : "text-brand-text-muted bg-brand-surface"}`}>
                    {idempotent ? "yes" : "no"}
                  </span>
                </td>
                <td className="py-2 pr-4">
                  <span className="text-[10px] font-mono text-brand-text-muted">{body ? "yes" : "no"}</span>
                </td>
                <td className="py-2 text-brand-text-muted">{use}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="text-[13px] text-brand-text-muted mb-6">
        <strong className="text-brand-text/80">Safe</strong> means the operation has no side effects.
        <strong className="text-brand-text/80 ml-2">Idempotent</strong> means calling it N times
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Status codes worth knowing</h3>

      <div className="space-y-4 mb-8">
        {STATUS_GROUPS.map(({ range, label, color, codes }) => (
          <div key={range} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2 border-b border-brand-border bg-brand-surface/30">
              <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border ${color}`}>{range}</span>
              <span className="text-[10px] uppercase tracking-wider text-brand-text-subtle">{label}</span>
            </div>
            <div className="divide-y divide-brand-border/50">
              {codes.map(({ code, name, when }) => (
                <div key={code} className="flex items-start gap-4 px-4 py-2.5">
                  <span className="font-mono text-[11px] font-semibold text-brand-text/80 w-8 shrink-0">{code}</span>
                  <span className="text-[11px] font-medium text-brand-text w-32 shrink-0">{name}</span>
                  <span className="text-[11px] text-brand-text-muted">{when}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">URL design principles</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose space-y-3">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Good vs bad URL patterns</p>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <p className="text-[9px] text-brand-destructive uppercase tracking-wider mb-2">Avoid</p>
            <pre className="text-[10px] font-mono bg-brand-destructive/5 border border-brand-destructive/20 rounded-lg p-3 space-y-1 leading-relaxed">
{`/getUser?id=42
/api/createPost
/users/getFollowers
/deleteComment/5
/api/v1/user_list`}
          ...
          <div>
            <p className="text-[9px] text-brand-primary uppercase tracking-wider mb-2">Prefer</p>
            <pre className="text-[10px] font-mono bg-brand-primary/5 border border-brand-primary/20 rounded-lg p-3 space-y-1 leading-relaxed">
{`GET /users/42
POST /posts
...
GET /users`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">API versioning strategies</h3>

      <div className="overflow-x-auto mb-6">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Strategy</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Example</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Pro</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Con</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {VERSIONING.map(({ strategy, example, pro, con }) => (
              <tr key={strategy}>
                <td className="py-2 pr-4 font-medium text-brand-text/80 align-top whitespace-nowrap">{strategy}</td>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-primary align-top">{example}</td>
                <td className="py-2 pr-4 text-brand-text-muted align-top">{pro}</td>
                <td className="py-2 text-brand-text-muted align-top">{con}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The over-fetching problem</h3>
