...
export function BeyondCAPSection() {
  return (
    <section>
      <h2 id="beyond-cap" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-6 mb-3">The PACELC model</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-5 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-4">PACELC framework</p>
        <div className="grid sm:grid-cols-2 gap-4 text-[11px]">
          <div className="p-3 rounded-xl bg-brand-surface space-y-2">
            <p className="font-semibold text-brand-text text-[12px]">During a Partition (P)</p>
            ...
            <div className="flex gap-2 text-[10px]">
              <span className="px-2 py-0.5 rounded bg-brand-primary/10 text-brand-primary font-mono">PA: serve stale</span>
              <span className="px-2 py-0.5 rounded bg-brand-warning/10 text-brand-warning font-mono">PC: reject writes</span>
            </div>
          </div>
          <div className="p-3 rounded-xl bg-brand-surface space-y-2">
            <p className="font-semibold text-brand-text text-[12px]">Else: during Normal Operation (E)</p>
            <p className="text-brand-text-muted">
            ...
            <div className="flex gap-2 text-[10px]">
              <span className="px-2 py-0.5 rounded bg-brand-primary/10 text-brand-primary font-mono">EL: fast, may lag</span>
              <span className="px-2 py-0.5 rounded bg-brand-warning/10 text-brand-warning font-mono">EC: slow, always fresh</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">System</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">PACELC</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Trade-off in plain terms</th>
            </tr>
          </thead>
            ...
            {PACELC_SYSTEMS.map(({ name, full, desc }) => (
              <tr key={name}>
                ...
                <td className="py-2 pr-4 align-top">
                  <span className="text-[9px] font-mono font-semibold text-brand-primary bg-brand-primary/10 px-1.5 py-0.5 rounded">
                    {full}
                  </span>
                </td>
                <td className="py-2 text-brand-text-muted">{desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Practical guidance</h3>
