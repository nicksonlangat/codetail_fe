...
const CAP_COLORS: Record<string, string> = {
  CP: "bg-brand-primary/10 text-brand-primary border-brand-primary/20",
  AP: "bg-brand-warning/10 text-brand-warning border-brand-warning/20",
};

export function CPvsAPSection() {
  return (
    <section>
      <h2 id="cp-vs-ap" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-6 mb-3">The consistency spectrum</h3>

      <div className="space-y-2 mb-8">
        {CONSISTENCY_LEVELS.map(({ name, aka, desc, systems, latency }) => (
          <div key={name} className="flex gap-4 p-3 rounded-xl border border-brand-border bg-white">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[12px] font-semibold text-brand-text">{name}</span>
                {aka && <span className="text-[9px] text-brand-text-muted bg-brand-surface px-1.5 py-0.5 rounded">{aka}</span>}
                <span className="ml-auto text-[9px] font-mono text-brand-text-muted">{latency} latency</span>
              </div>
              <p className="text-[11px] text-brand-text-muted leading-relaxed mb-1">{desc}</p>
              <p className="text-[10px] text-brand-text-muted">
                <span className="font-medium text-brand-text-muted">Examples: </span>
                {systems}
              </p>
            </div>
          </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Where real systems sit</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              ...
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">CAP</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Behavior during partition</th>
            </tr>
          </thead>
