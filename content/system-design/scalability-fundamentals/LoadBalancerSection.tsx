...
export function LoadBalancerSection() {
  return (
    <section>
      <h2 id="load-balancer-algorithms" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="space-y-3 mb-8">
        {[
          ...
          },
          {
            ...
            icon: "⚖️",
            desc: "Like Round Robin, but servers with higher weight receive proportionally more requests. Useful when servers have different hardware specs, a 16-core server gets 2x the traffic of an 8-core one.",
          ...
          <div key={name} className="p-4 rounded-xl border border-brand-border bg-white">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-base">{icon}</span>
              <span className="text-[13px] font-semibold text-brand-text">{name}</span>
            </div>
            <p className="text-[12px] text-brand-text-muted mb-3 leading-relaxed">{desc}</p>
            <div className="grid grid-cols-2 gap-2 text-[10px]">
              <div>
                <span className="text-brand-primary font-medium">Best for: </span>
                <span className="text-brand-text-muted">{best}</span>
              </div>
              <div>
                <span className="text-brand-warning font-medium">Avoid when: </span>
                <span className="text-brand-text-muted">{worst}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">See it in action: the simulator</h3>
