...
export function ChoosingSection() {
  return (
    <section>
      <h2 id="choosing-a-database" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Decision guide</h3>

      <div className="overflow-x-auto mb-8">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium w-1/3">Question</th>
              <th className="text-left py-2 pr-3 text-brand-chart-2 font-medium">SQL answer</th>
              <th className="text-left py-2 text-brand-primary font-medium">NoSQL answer</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {DECISION_ROWS.map(({ question, sql, nosql, winner }) => (
              <tr key={question} className={winner === "sql" ? "bg-brand-chart-2/2" : winner === "nosql" ? "bg-brand-primary/2" : ""}>
                <td className="py-2.5 pr-4 font-medium text-brand-text/80 align-top">{question}</td>
                <td className={`py-2.5 pr-3 align-top ${winner === "sql" ? "text-brand-chart-2 font-medium" : "text-brand-text-muted"}`}>
                  {sql}
                </td>
                <td className={`py-2.5 align-top ${winner === "nosql" ? "text-brand-primary font-medium" : "text-brand-text-muted"}`}>
                  {nosql}
                </td>
              </tr>
            ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Common use cases and their databases</h3>

      <div className="grid gap-2 sm:grid-cols-2">
        {USE_CASES.map(({ usecase, pick, reason }) => (
          <div key={usecase} className="p-3 rounded-xl border border-brand-border bg-white">
            <div className="flex items-start justify-between gap-2 mb-1">
              <span className="text-[12px] font-semibold text-brand-text">{usecase}</span>
              <span className="text-[9px] font-mono text-brand-primary bg-brand-primary/10 px-1.5 py-0.5 rounded shrink-0">
                {pick}
              </span>
            </div>
            <p className="text-[11px] text-brand-text-muted">{reason}</p>
          </div>
        ))}
      </div>

      <div className="border-l-2 border-brand-warning bg-brand-warning/5 pl-4 py-3 rounded-r-lg mt-6">
