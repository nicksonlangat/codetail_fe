...
export function DNSSection() {
  return (
    <section>
      <h2 id="dns-resolution" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Step 1: DNS Resolution
      </h2>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        The internet routes traffic using IP addresses (like <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">142.250.80.46</code>),
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The caching hierarchy</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">TTL: the knob that controls freshness vs speed</h3>
        ...
        <table className="w-full text-[12px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">TTL</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Use case</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Trade-off</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {[
              ...
              <tr key={row.ttl}>
                <td className="py-2 pr-4 font-mono text-brand-primary">{row.ttl}</td>
                <td className="py-2 pr-4 text-brand-text/80">{row.use}</td>
                <td className="py-2 text-brand-text-muted">{row.tradeoff}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">DNS as a routing tool</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-6">
        {[
          { icon: "🌍", label: "GeoDNS", desc: "Returns different IPs per region. EU users go to Frankfurt. US users go to Virginia. Reduces latency by 100ms+ for global services." },
          ...
          { icon: "🛡️", label: "DNS Failover", desc: "Health checks trigger automatic DNS changes when servers go down. Recovery time equals TTL. Set short TTLs before planned maintenance." },
        ].map(({ icon, label, desc }) => (
