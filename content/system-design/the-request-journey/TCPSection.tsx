...
export function TCPSection() {
  return (
    <section>
      <h2 id="tcp-tls" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Why the handshake costs a round trip</h3>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        Each round trip (client to server to client) takes time proportional to the physical distance. A
        ...
        <table className="w-full text-[12px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Route</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Distance</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Min RTT</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {[
              { route: "NYC to NYC (same city)", dist: "~50km", rtt: "~1ms" },
              { route: "NYC to LA", dist: "~4,500km", rtt: "~40ms" },
              { route: "NYC to London", dist: "~5,500km", rtt: "~70ms" },
              { route: "NYC to Sydney", dist: "~16,000km", rtt: "~180ms" },
            ].map((r) => (
              <tr key={r.route}>
                <td className="py-2 pr-4 text-brand-text/80">{r.route}</td>
                <td className="py-2 pr-4 text-brand-text-muted font-mono">{r.dist}</td>
                <td className="py-2 font-mono text-brand-primary">{r.rtt}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">TLS: adding encryption on top of TCP</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-3 mb-6">
        {[
          ...
          <div key={label} className="p-4 bg-white border border-brand-border rounded-xl">
            ...
            <div className="text-[20px] font-bold text-brand-primary mb-1">{cost}</div>
            <div className="text-[10px] text-brand-text-muted">{note}</div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Connection reuse: the real optimization</h3>
