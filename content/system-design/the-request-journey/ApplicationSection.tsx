export function ApplicationSection() {
  return (
    <section>
      <h2 id="application-layer" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The load balancer</h3>

      <div className="bg-white border border-brand-border rounded-xl p-5 mb-6 not-prose">
        <div className="flex items-center gap-4 mb-4">
          <div className="shrink-0 w-12 h-12 rounded-xl bg-brand-primary/10 border border-brand-primary/20 flex items-center justify-center text-2xl">⚖️</div>
          <div>
            <div className="text-[13px] font-semibold text-brand-text">Load Balancer</div>
            <div className="text-[11px] text-brand-text-muted">Reverse proxy that distributes requests across backend servers</div>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[11px]">
          {[
            ...
            <div key={label} className="p-3 rounded-lg bg-brand-surface/60">
              <div className="font-semibold text-brand-text mb-1">{label}</div>
              <div className="text-brand-text-muted leading-relaxed">{desc}</div>
            </div>
          ))}
        </div>
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The application server</h3>
      ...
      <div className="space-y-2 mb-6">
        {[
          ...
          <div key={step} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            ...
            <div>
              ...
              <span className="text-[12px] text-brand-text-muted">{desc}</span>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The database: the slow step</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-6">
        {[
          ...
          <div key={label} className={`p-4 rounded-xl border ${bad ? "border-brand-destructive/30 bg-brand-destructive/5" : "border-brand-primary/30 bg-brand-primary/5"}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-base">{icon}</span>
              <span className="text-[12px] font-semibold text-brand-text">{label}</span>
            </div>
            <pre className="text-[10px] font-mono bg-brand-surface rounded p-2 mb-2 whitespace-pre-wrap">{code}</pre>
            <p className="text-[10px] text-brand-text-muted">{note}</p>
          </div>
        ))}
      </div>
      ...
      <div className="border-l-2 border-brand-warning bg-brand-warning/5 pl-4 py-3 rounded-r-lg">
        <p className="text-[13px] text-brand-text-muted">
          ...
          separately equals 101 queries. Use a JOIN or batch fetch instead. N+1 queries are the most common
