...
export function CircuitBreakerSection() {
  return (
    <section>
      <h2 id="circuit-breaker" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Circuit Breaker
      </h2>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-4 mb-3">The three states</h3>
      ...
      <div className="space-y-3 mb-8">
        {[
          ...
          <div key={state} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            ...
            <div className="px-4 py-3 space-y-1.5 text-[11px]">
              ...
              <p className="font-mono text-[10px] text-brand-text-subtle">→ {transition}</p>
            </div>
          </div>
        ))}
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Implementation with a fallback</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Python circuit breaker with graceful degradation</p>
...
def get_fallback_recommendations() -> list[str]:
    ...
    return popular_items_cache.get() or []`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What triggers a circuit breaker</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {[
          ...
          <div key={trigger} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{trigger}</p>
