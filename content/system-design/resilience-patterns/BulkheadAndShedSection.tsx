...
export function BulkheadAndShedSection() {
  return (
    <section>
      <h2 id="bulkhead-shed" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Bulkheads, Load Shedding, and Graceful Degradation
      </h2>

      <h3 className="text-base font-semibold text-brand-text mt-6 mb-3">Bulkhead pattern</h3>
...
# payments pool (10 threads) is completely unaffected`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Load shedding</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Graceful degradation</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Feature</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Degraded behavior</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Justification</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {DEGRADATION_EXAMPLES.map(({ feature, degraded, justification }) => (
              <tr key={feature}>
                <td className="py-2.5 pr-4 font-medium text-brand-text/80 align-top">{feature}</td>
                <td className={`py-2.5 pr-4 align-top font-mono text-[10px] ${degraded.startsWith("Do NOT") ? "text-brand-destructive" : "text-brand-primary"}`}>
                  {degraded}
                </td>
                <td className="py-2.5 text-brand-text-muted align-top">{justification}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Health checks and readiness</h3>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={check} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{check}</p>
              <p className="text-[11px] text-brand-text-muted">{detail}</p>
            </div>
          </div>
        ))}
