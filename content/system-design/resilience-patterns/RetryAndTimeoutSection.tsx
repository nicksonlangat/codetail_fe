...
export function RetryAndTimeoutSection() {
  return (
    <section>
      <h2 id="retry-timeout" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Timeout types</h3>
        ...
        {TIMEOUT_TYPES.map(({ type, value, desc, when }) => (
          <div key={type} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{type}</span>
              <span className="ml-auto font-mono text-[10px] text-brand-primary">{value}</span>
            </div>
            <div className="px-4 py-3 space-y-1 text-[11px]">
              <p className="text-brand-text-muted">{desc}</p>
              <p>
                <span className="font-medium text-brand-text/80">When: </span>
                <span className="text-brand-text-muted">{when}</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Retry strategies</h3>
      ...
      <div className="space-y-3 mb-6">
        {RETRY_PATTERNS.map(({ name, formula, example, desc }) => (
          <div key={name} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{name}</span>
              <span className="font-mono text-[9px] text-brand-text-muted">{formula}</span>
            </div>
            <div className="px-4 py-3 space-y-1.5">
              <div className="flex gap-1.5 font-mono text-[9px]">
                {example.split(", ").map((v, i) => (
                  <span key={i} className="px-2 py-1 bg-brand-surface rounded text-brand-text-muted">
                    {v}
                  </span>
                ))}
              </div>
...
async def call_with_retry(
    ...
    raise RuntimeError("unreachable")`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What NOT to retry</h3>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={c} className="flex gap-3 p-3 rounded-xl border border-brand-warning/20 bg-brand-warning/5">
            <span className="text-brand-warning font-bold text-sm shrink-0 mt-0.5">!</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{c}</p>
