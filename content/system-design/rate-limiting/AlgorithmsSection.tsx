...
export function AlgorithmsSection() {
  return (
    <section>
      <h2 id="algorithms" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-4 mb-3">Token bucket</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Token bucket in Redis (Lua script)</p>
...
if tokens >= cost then
  ...
  return 0   -- rejected
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Fixed window</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Fixed window in Redis</p>
        <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-4 py-3 overflow-x-auto leading-relaxed">
{`def is_allowed(user_id: str, limit: int, window_sec: int) -> bool:
    ...
    return count <= limit`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Sliding window</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Algorithm comparison</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              {["Algorithm", "Burst", "Memory", "Accuracy", "Complexity", "Best for"].map((h) => (
                <th key={h} className="text-left py-2 pr-3 text-brand-text-muted font-medium whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-brand-border/50">
            {ALGORITHM_COMPARISON.map(({ algo, burst, memory, accuracy, complexity, bestFor }) => (
              <tr key={algo}>
                <td className="py-2.5 pr-3 font-semibold text-brand-text/80 align-top whitespace-nowrap">{algo}</td>
                <td className="py-2.5 pr-3 text-brand-text-muted align-top">{burst}</td>
                <td className="py-2.5 pr-3 font-mono text-[10px] align-top">{memory}</td>
                <td className="py-2.5 pr-3 text-brand-text-muted align-top">{accuracy}</td>
                <td className="py-2.5 pr-3 text-brand-text-muted align-top">{complexity}</td>
                <td className="py-2.5 text-brand-text-muted align-top">{bestFor}</td>
              </tr>
            ))}
          </tbody>
        </table>
