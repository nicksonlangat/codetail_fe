const THREAT_SCENARIOS = [
  ...
  },
  {
    ...
    without: "A bug in a client retries a failing endpoint in a tight loop. 50 clients x 1000 req/sec = 50,000 req/sec hitting your API. Database falls over.",
...
export function WhyRateLimitSection() {
  return (
    <section>
      <h2 id="why-rate-limit" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What happens without rate limiting</h3>

      <div className="space-y-3 mb-8">
        {THREAT_SCENARIOS.map(({ threat, without, with: w, severity }) => (
          <div key={threat} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${severity === "high" ? "bg-brand-destructive" : "bg-brand-warning"}`} />
              <span className="text-[11px] font-semibold text-brand-text">{threat}</span>
            </div>
            <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-brand-border/50">
              <div className="px-4 py-3">
                <p className="text-[9px] uppercase tracking-wider text-brand-destructive mb-1">Without rate limiting</p>
                <p className="text-[11px] text-brand-text-muted">{without}</p>
              </div>
              <div className="px-4 py-3">
                <p className="text-[9px] uppercase tracking-wider text-brand-primary mb-1">With rate limiting</p>
                <p className="text-[11px] text-brand-text-muted">{w}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Dimensions of rate limiting</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-6">
        {[
          ...
          <div key={dimension} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{dimension}</p>
