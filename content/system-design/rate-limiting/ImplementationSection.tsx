...
export function ImplementationSection() {
  return (
    <section>
      <h2 id="implementation" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Application middleware pattern (FastAPI)</h3>
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    return response`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">429 response best practices</h3>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Standard response headers</h3>

      <div className="overflow-x-auto mb-8">
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Header</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Example</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Meaning</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {HEADER_CONVENTIONS.map(({ header, value, desc }) => (
              <tr key={header}>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-primary align-top">{header}</td>
                <td className="py-2 pr-4 font-mono text-[10px] text-brand-text/80 align-top">{value}</td>
                <td className="py-2 text-brand-text-muted align-top">{desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Where to enforce rate limits</h3>

      <div className="space-y-3">
        {PLACEMENT_OPTIONS.map(({ location, examples, pro, con, when }) => (
          <div key={location} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <p className="text-[11px] font-semibold text-brand-text">{location}</p>
              <p className="text-[9px] text-brand-text-muted font-mono">{examples}</p>
            </div>
            <div className="px-4 py-3 space-y-1 text-[11px]">
              <p>
                <span className="font-medium text-brand-primary">Pro: </span>
                <span className="text-brand-text-muted">{pro}</span>
              </p>
              <p>
                <span className="font-medium text-brand-warning">Con: </span>
                <span className="text-brand-text-muted">{con}</span>
              </p>
              <p>
                <span className="font-medium text-brand-text/80">Use when: </span>
