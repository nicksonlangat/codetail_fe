...
export function CoreDesignSection() {
  return (
    <section>
      <h2 id="core-design" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Core Design
      </h2>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Code generation</h3>
      ...
      <div className="space-y-3 mb-6">
        {CODE_GEN.map(({ approach, how, tradeoff, verdict }) => (
          <div key={approach} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{approach}</span>
              <span className="text-[9px] font-mono text-brand-primary">{verdict.split(".")[0]}</span>
            </div>
            <div className="px-4 py-3 space-y-1.5 text-[11px]">
              <p className="text-brand-text-muted">
                <span className="font-medium text-brand-text/80">How: </span>
                {how}
              </p>
              <p className="text-brand-text-muted">
                <span className="font-medium text-brand-text/80">Tradeoff: </span>
                {tradeoff}
              </p>
            </div>
          </div>
...
async def create_short_url(long_url: str, user_id: int) -> str:
    ...
    return short_code`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Schema design</h3>

      <div className="bg-white border border-brand-border rounded-xl p-4 mb-6 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">PostgreSQL schema</p>
        <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-4 py-3 overflow-x-auto leading-relaxed">
{`CREATE TABLE urls (
...
CREATE INDEX idx_urls_expires_at ON urls (expires_at)
    WHERE expires_at IS NOT NULL;`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The redirect API: 301 vs 302</h3>
      ...
      <div className="bg-white border border-brand-border rounded-xl p-4 not-prose">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">Redirect handler with cache-aside</p>
        <pre className="text-[10px] font-mono bg-brand-surface rounded-xl px-4 py-3 overflow-x-auto leading-relaxed">
