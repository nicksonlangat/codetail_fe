...
export function ScalingSection() {
  return (
    <section>
      <h2 id="scaling" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Read path: layered caching</h3>

      <div className="space-y-2 mb-8">
        {READ_PATH.map(({ layer, detail, latency }) => (
          <div key={layer} className="bg-white border border-brand-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-brand-border bg-brand-surface/30">
              <span className="text-[11px] font-semibold text-brand-text">{layer}</span>
              <span className="font-mono text-[9px] text-brand-primary">{latency}</span>
            </div>
...
{`async def delete_url(short_code: str, user_id: int):
    ...
    asyncio.create_task(cdn_purge(f"/{short_code}"))`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Write path: rate limiting and deduplication</h3>
      ...
      <div className="space-y-2 mb-6">
        {[
          ...
          <div key={title} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{title}</p>
              <p className="text-[11px] text-brand-text-muted">{detail}</p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Database: indexes and read replicas</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {[
          ...
          <div key={label} className="bg-white border border-brand-border rounded-xl p-3">
            <p className="text-[11px] font-semibold text-brand-text mb-2">{label}</p>
            <ul className="space-y-1">
              {items.map((item) => (
                <li key={item} className="text-[11px] text-brand-text-muted flex gap-2">
