...
export function ProductionSection() {
  return (
    <section>
      <h2 id="production" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Production: Analytics, Resilience, and What We Left Out
      </h2>

      <h3 className="text-base font-semibold text-brand-text mt-6 mb-3">Analytics pipeline</h3>
...
# Materialised view aggregates: clicks_by_day, clicks_by_country`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Resilience</h3>
...
async def resolve_url(short_code: str) -> str:
    ...
    return long_url`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The interview framework</h3>
      ...
      <div className="space-y-2 mb-8">
        {INTERVIEW_STEPS.map(({ step, detail }) => (
          <div key={step} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <span className="text-brand-primary font-bold text-sm shrink-0 mt-0.5">→</span>
            <div>
              <p className="text-[12px] font-semibold text-brand-text mb-0.5">{step}</p>
              <p className="text-[11px] text-brand-text-muted">{detail}</p>
            </div>
          </div>
        ))}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">What we left out</h3>
      ...
      <div className="space-y-2">
        {LEFT_OUT.map(({ item, why }) => (
          <div key={item} className="flex gap-3 p-3 rounded-xl border border-brand-border/50 bg-brand-surface/30">
            <span className="text-brand-text-muted font-bold text-sm shrink-0 mt-0.5">–</span>
            <div>
              <p className="text-[12px] font-semibold mb-0.5 text-brand-text/70">{item}</p>
