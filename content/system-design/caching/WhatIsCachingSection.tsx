...
const TIER_COLORS: Record<string, { bar: string; text: string }> = {
  extreme: { bar: "bg-brand-primary", text: "text-brand-primary" },
  fast: { bar: "bg-brand-success", text: "text-brand-success" },
  ok: { bar: "bg-brand-warning", text: "text-brand-warning" },
  slow: { bar: "bg-brand-destructive", text: "text-brand-destructive" },
};
...
export function WhatIsCachingSection() {
  return (
    <section>
      <h2 id="why-caching-matters" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Why Caching Matters
      </h2>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The memory hierarchy</h3>

      <div className="bg-white border border-brand-border rounded-xl p-5 mb-8 not-prose space-y-2">
          ...
          return (
            <div key={level} className="space-y-1">
              <div className="flex items-center justify-between text-[10px]">
                <span className="font-medium text-brand-text/80 w-40 shrink-0">{level}</span>
                <span className={`font-mono font-semibold w-20 text-right shrink-0 ${colors.text}`}>{latency}</span>
                <span className="text-brand-text-subtle text-[9px] hidden sm:block ml-3">{note}</span>
              </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Cache layers in a web system</h3>
        ...
        {CACHE_LAYERS.map(({ icon, label, latency, desc }, i) => (
          <div key={label} className="flex gap-3 p-3 rounded-xl border border-brand-border bg-white">
            <div className="flex items-center gap-2 w-8 shrink-0">
              <span className="text-[10px] font-mono text-brand-text-subtle w-4">{i + 1}</span>
              <span>{icon}</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-[12px] font-semibold text-brand-text">{label}</span>
                <span className="text-[9px] font-mono text-brand-text-muted bg-brand-surface px-1.5 py-0.5 rounded">
                  {latency}
                </span>
              </div>
