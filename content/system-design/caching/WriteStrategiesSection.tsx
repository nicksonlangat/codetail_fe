const STRATEGIES = [
  ...
  },
  {
    ...
    readSteps: [{ label: "App checks cache", result: "Always a HIT (if key exists)." }],
    writeSteps: [
  ...
  },
  {
    ...
    readSteps: [{ label: "App checks cache", result: "HIT: serves latest data including unflushed writes." }],
    writeSteps: [
...
export function WriteStrategiesSection() {
  return (
    <section>
      <h2 id="write-strategies" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Write Strategies
      </h2>
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-8">
        ...
        {STRATEGIES.map(({ name, aka, description, readSteps, writeSteps, pros, cons, verdict, highlight }) => (
          <div
            ...
            className={`rounded-xl border overflow-hidden ${
              highlight ? "border-brand-primary/30 bg-brand-primary/5" : "border-brand-border bg-white"
            }`}
          >
            <div
              className={`px-4 py-3 border-b flex items-center gap-2 ${
                highlight ? "border-brand-primary/20 bg-brand-primary/5" : "border-brand-border bg-brand-surface/50"
              }`}
            >
              <span className="text-[13px] font-semibold text-brand-text">{name}</span>
              {aka && (
                <span className="text-[10px] text-brand-text-muted bg-brand-surface px-2 py-0.5 rounded">{aka}</span>
              )}
              {highlight && (
                <span className="ml-auto text-[9px] font-medium text-brand-primary bg-brand-primary/10 px-2 py-0.5 rounded-full">
                  recommended default
                </span>
              )}
            </div>
            ...
            <div className="p-4 space-y-4">
              <p className="text-[12px] text-brand-text-muted leading-relaxed">{description}</p>

              <div className="grid sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-2">Read path</p>
                  {readSteps.map((s, i) => (
                    <div key={i} className="flex gap-2 text-[11px]">
                      <span className="text-[9px] font-mono text-brand-text-subtle mt-0.5 shrink-0">{i + 1}</span>
                      <div>
                        <span className="font-medium text-brand-text/80">{s.label}</span>
                        <span className="text-brand-text-muted">: {s.result}</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="space-y-1">
                  <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-2">Write path</p>
                  {writeSteps.map((s, i) => (
                    <div key={i} className="flex gap-2 text-[11px]">
                      <span className="text-[9px] font-mono text-brand-text-subtle mt-0.5 shrink-0">{i + 1}</span>
                      <div>
              ...
              <div className="grid sm:grid-cols-2 gap-3 text-[11px]">
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-brand-primary mb-1.5">Strengths</p>
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-medium text-brand-warning mb-1.5">Weaknesses</p>
                  <ul className="space-y-1">
                    {cons.map((c) => (
                      <li key={c} className="flex gap-1.5 text-brand-text-muted">
                        <span className="text-brand-warning shrink-0">-</span>
                        {c}
                      </li>
                    ))}
                  </ul>
              ...
              <p className="text-[11px] border-t border-brand-border/50 pt-3 text-brand-text-muted">
                <span className="font-semibold text-brand-text">When to use: </span>
                {verdict}
              </p>
            </div>
          </div>
