...
export function InvalidationSection() {
  return (
    <section>
      <h2 id="cache-invalidation" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Cache Invalidation
      </h2>
        ...
        {STRATEGIES.map(({ name, icon, how, tradeoff, code }) => (
          <div key={name} className="rounded-xl border border-brand-border bg-white overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              <p className="text-[12px] text-brand-text-muted leading-relaxed">{how}</p>
              ...
              <p className="text-[11px] text-brand-text-muted border-t border-brand-border/50 pt-2">
                <span className="font-semibold text-brand-text">Trade-off: </span>
                {tradeoff}
              </p>
            </div>
          </div>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Choosing a TTL</h3>
        ...
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="border-b border-brand-border">
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Resource</th>
              <th className="text-left py-2 pr-4 text-brand-text-muted font-medium">Typical TTL</th>
              <th className="text-left py-2 text-brand-text-muted font-medium">Rationale</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-border/50">
            {TTL_EXAMPLES.map(({ resource, ttl, rationale }) => (
              <tr key={resource}>
                <td className="py-2 pr-4 font-medium text-brand-text/80 align-top">{resource}</td>
                <td className="py-2 pr-4 font-mono text-brand-primary align-top whitespace-nowrap">{ttl}</td>
                <td className="py-2 text-brand-text-muted">{rationale}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">The cache stampede problem</h3>
...
if now - computed_at > ttl - beta * log(rand()):
    ...
    with cache.lock(key, timeout=2):
        if not cache.get(key):  # re-check
          ...
          <div key={name} className="p-4 rounded-xl border border-brand-border bg-white space-y-2">
            <p className="text-[12px] font-semibold text-brand-text">{name}</p>
            <p className="text-[11px] text-brand-text-muted leading-relaxed">{desc}</p>
            <pre className="text-[9px] font-mono bg-brand-surface rounded px-2 py-1.5 overflow-x-auto whitespace-pre">
              {code}
            </pre>
          </div>
        ))}
