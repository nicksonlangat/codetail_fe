...
export function EverythingIsAnObjectSection() {
  return (
    <section>
      <h2 id="everything-is-an-object" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
      ...
      <div className="grid gap-3 sm:grid-cols-3 mb-8">
        {OBJECT_PROPS.map(({ label, fn, desc }) => (
          <div key={label} className="bg-white border border-brand-border rounded-xl p-4">
            <p className="text-[11px] uppercase tracking-wider font-medium text-brand-text-subtle mb-1">
              {label}
            </p>
            <code className="font-mono text-[13px] text-brand-primary font-semibold block mb-2">{fn}</code>
            <p className="text-[12px] text-brand-text-muted leading-relaxed">{desc}</p>
          </div>
        ))}
      </div>
...
140234567890`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Identity vs equality</h3>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-6">
        <div className="bg-white border border-brand-border rounded-xl p-4">
          <code className="font-mono text-[13px] text-brand-primary font-semibold block mb-2">a == b</code>
        ...
        <div className="bg-white border border-brand-border rounded-xl p-4">
          <code className="font-mono text-[13px] text-brand-primary font-semibold block mb-2">a is b</code>
          <p className="text-[12px] text-brand-text-muted leading-relaxed">
...
True`}
      ...
      <div className="border-l-2 border-brand-warning bg-brand-warning/5 pl-4 py-3 rounded-r-lg mt-6">
        <p className="text-[13px] text-brand-text-muted">
          <strong>Rule:</strong> use <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">==</code> to
          compare values. Use{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">is</code> only when
          you specifically mean "same object", most commonly{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">x is None</code>.
          Using <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">is</code> to
