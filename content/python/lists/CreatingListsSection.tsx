...
export function CreatingListsSection() {
  return (
    <section>
      <h2 id="creating-lists" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Creating lists
      </h2>
...
print(matrix[1][2])   # 6: row 1, column 2`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">
        <code className="font-mono text-brand-primary">list()</code> constructor
      </h3>
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
...
print(evens)    # [0, 2, 4, 6, 8]`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Initializing with a default value</h3>
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        The{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">*</code>{" "}
        operator repeats a list. Use it to pre-fill a list with a known size. Works for
        immutables only. See the gotcha below.
      </p>
      <CodeBlock
        code={`zeros = [0] * 5
...
[False, False, False]`}
      />
      <div className="border-l-2 border-brand-warning bg-brand-warning/5 pl-4 py-3 rounded-r-lg mt-3">
        <p className="text-[13px] text-brand-text-muted">
          <strong>Gotcha:</strong>{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">[[]] * 3</code>{" "}
          ...
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">[[] for _ in range(3)]</code>.
        </p>
      </div>
    </section>
  );
