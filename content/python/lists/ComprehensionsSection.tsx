...
export function ComprehensionsSection() {
  return (
    <section>
      <h2 id="comprehensions" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        List comprehensions
      </h2>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        A list comprehension builds a new list from an <Term term="iterable">iterable</Term>{" "}
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        ...
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">.append()</code>{" "}
...
print(squares)    # [0, 1, 4, 9, 16, 25]`}
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mb-6">
        <p className="text-[13px] text-brand-text-muted italic">
          ...
          . The{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">if condition</code>{" "}
          part is optional.
        </p>
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">With a filter condition</h3>
      <CodeBlock
        code={`nums = range(20)
...
['hello', 'world', 'python']`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Nested comprehensions</h3>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        When a comprehension has multiple{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">for</code>{" "}
...
[('red', 'S'), ('red', 'M'), ('red', 'L'), ('green', 'S'), ('green', 'M'), ('green', 'L')]`}
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mt-4">
        <p className="text-[13px] font-semibold text-brand-text/80 mb-2">Formula</p>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-medium text-brand-text-subtle mb-1.5">Nested loops</p>
            <pre className="font-mono text-[12px] text-brand-text/80 leading-relaxed bg-brand-surface px-3 py-2.5 rounded-lg">{`for row in matrix:   # outer
    for x in row:    # inner
        result.append(x)`}</pre>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-medium text-brand-text-subtle mb-1.5">Comprehension</p>
            <pre className="font-mono text-[12px] text-brand-text/80 leading-relaxed bg-brand-surface px-3 py-2.5 rounded-lg">{`[x                   # expression
  ...
  for x in row]      # inner`}</pre>
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">When not to use comprehensions</h3>
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
...
for item in get_items():
    ...
    result.append(process(item))`}
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mt-3">
        <p className="text-[13px] text-brand-text-muted italic">
