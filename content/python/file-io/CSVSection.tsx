...
export function CSVSection() {
  return (
    <section>
      <h2 id="csv" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        CSV files
      </h2>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        Python&apos;s{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">csv</code>{" "}
...
with open("scores.csv", encoding="utf-8") as f:
    for row in csv.DictReader(f):
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Writing CSV</h3>
    ...
    writer.writerows(students)`}
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mt-2">
        <p className="text-[13px] text-brand-text-muted italic">
          Always pass{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">newline=&quot;&quot;</code>{" "}
          when opening a file for{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">csv.writer</code>.
