...
export function PerformanceSection() {
  return (
    <section>
      <h2 id="performance" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Performance
      </h2>
...
for word in words:
    result += word + " "   # creates a brand new string every time`}
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mt-6 mb-4">
        ...
        copies the entire existing string plus the new piece into a new object. For a loop over n
        words, you copy 1 char, then 2, then 3, and so on. The total work is proportional to n².
        Fine for 10 items, genuinely painful at 10,000.
      </p>
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        The fix is{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">str.join()</code>.
...
result = buf.getvalue()`}
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mt-6">
        <p className="text-[11px] text-brand-primary font-medium mb-1">Performance note</p>
        <p className="text-[13px] text-brand-text-muted">
          CPython has an optimization that sometimes makes{" "}
          <code className="font-mono bg-brand-surface px-1 rounded">+=</code> reuse the buffer
          when the string has only one reference. Don&apos;t rely on it. The optimization
          disappears with aliasing and isn&apos;t guaranteed. Use{" "}
          <code className="font-mono bg-brand-surface px-1 rounded">join()</code> and never think
          about it again.
        </p>
      </div>
    </section>
