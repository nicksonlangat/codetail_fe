...
const SLICE_PARTS: [string, string][] = [
...
export function SlicingSection() {
  return (
    <section>
      <h2 id="indexing-slicing" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Indexing &amp; slicing
      </h2>
      ...
      <div className="space-y-2 mb-6 mt-8">
        {SLICE_PARTS.map(([term, desc]) => (
          <div key={term} className="flex gap-3 items-baseline">
            <code className="font-mono text-[12px] bg-brand-surface px-2 py-0.5 rounded text-brand-primary shrink-0">
              {term}
            </code>
            <span className="text-[13px] text-brand-text/80">{desc}</span>
          </div>
        ))}
      </div>

      <div className="border-l-2 border-brand-warning bg-brand-warning/5 pl-4 py-3 rounded-r-lg mb-6">
        <p className="text-[11px] text-brand-warning font-medium mb-1">Gotcha</p>
        <p className="text-[13px] text-brand-text-muted">
          <code className="font-mono bg-brand-surface px-1 rounded">s[start:stop]</code> includes{" "}
          <code className="font-mono bg-brand-surface px-1 rounded">start</code> but excludes{" "}
          <code className="font-mono bg-brand-surface px-1 rounded">stop</code>. So{" "}
          <code className="font-mono bg-brand-surface px-1 rounded">s[0:5]</code> gives characters
          at indices 0, 1, 2, 3, 4. Never 5.
        </p>
      </div>

      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Patterns you&apos;ll use constantly</h3>
      <CodeBlock
        code={`s = "Hello, World!"
...
l`}
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mt-6">
        ...
        single indices (like{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">s[99]</code>{" "}
