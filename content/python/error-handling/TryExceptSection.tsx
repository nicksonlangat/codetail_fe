...
export function TryExceptSection() {
  return (
    <section>
      <h2 id="try-except" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        try / except
      </h2>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        Wrap code that might fail in a{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">try</code>{" "}
        block. If an exception is raised, Python jumps to the matching{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">except</code>{" "}
...
expected a string, got NoneType`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">Accessing the exception object</h3>
...
ValueError: invalid literal for int() with base 10: 'bad'`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">else and finally</h3>
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        ...
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">finally</code>{" "}
    ...
    try:
      ...
      <div className="border-l-2 border-brand-primary bg-brand-primary/5 pl-4 py-3 rounded-r-lg mt-2">
        <p className="text-[13px] text-brand-text-muted italic">
          In practice, use{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">with open(path) as f</code>{" "}
          instead of manual{" "}
          <code className="font-mono text-[12px] bg-brand-surface px-1 py-0.5 rounded">finally: f.close()</code>.
