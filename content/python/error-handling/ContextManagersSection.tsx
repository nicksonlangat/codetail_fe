...
export function ContextManagersSection() {
  return (
    <section>
      <h2 id="context-managers" className="text-xl font-semibold text-brand-text mt-12 mb-4 scroll-mt-24">
        Context managers
      </h2>
      ...
      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        ...
        the{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">try/finally</code>{" "}
...
with open("input.txt") as src, open("output.txt", "w") as dst:
    dst.write(src.read().upper())`}
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">contextlib.contextmanager</h3>

      <p className="text-[15px] leading-relaxed text-brand-text/90 mb-4">
        The{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">@contextmanager</code>{" "}
        decorator lets you write a context manager as a generator. Everything before
        the{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">yield</code>{" "}
        is setup (called on entry), the yielded value becomes the{" "}
        <code className="font-mono text-[13px] bg-brand-surface px-1.5 py-0.5 rounded">as</code>{" "}
...
def managed_connection(host: str):
    ...
    try:
      ...
      <h3 className="text-base font-semibold text-brand-text mt-8 mb-3">contextlib utilities</h3>
