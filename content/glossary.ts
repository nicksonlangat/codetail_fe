...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  unique: {
    ...
    type: "Python concept",
    definition:
      "Appearing at most once in a collection. Sets enforce uniqueness automatically: adding a value that's already present is a silent no-op, not an error. This is what makes set(some_list) the standard one-line way to deduplicate.",
    example: `s = {1, 2, 2, 3}
print(s)          # {1, 2, 3}, duplicate 2 collapses

s.add(3)          # already present, no-op
print(s)          # still {1, 2, 3}`,
    exampleOutput: `{1, 2, 3}
{1, 2, 3}`,
  },
  key: {
    ...
    type: "Python concept",
    definition:
      "The lookup value in a dictionary, mapped to an associated value. Keys must be hashable (strings, numbers, tuples of hashables) and unique within a dict; assigning to an existing key overwrites its value rather than creating a duplicate.",
    example: `user = {"name": "Alice", "age": 30}
print(user["name"])   # "name" is the key, "Alice" is the value

user["age"] = 31      # same key, value overwritten
print(user)`,
    exampleOutput: `Alice
{'name': 'Alice', 'age': 31}`,
  },
  comma: {
    ...
    type: "Python syntax",
    definition:
      "The comma, not the parentheses, is what creates a tuple. Parentheses just group the expression for readability. A trailing comma after a single value creates a one-element tuple; without it, Python just evaluates the parenthesized expression as-is.",
    example: `point = 3, 7        # comma alone creates a tuple
print(type(point))   # <class 'tuple'>

single = (42,)        # comma makes this a tuple
not_tuple = (42)      # no comma, just the int 42`,
    exampleOutput: `<class 'tuple'>`,
  },
  sequence: {
    name: "sequence",
    type: "Python concept",
...
True`,
  ...
  "guard clause": {
    ...
    type: "Python concept",
    definition:
      "An early return on an edge case, placed at the top of a function before the main logic. Guard clauses trade nested if blocks for a flat sequence of exits, so the reader only has to hold one condition in their head at a time instead of tracking several levels of indentation.",
