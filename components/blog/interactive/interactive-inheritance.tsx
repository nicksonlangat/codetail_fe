...
const colorCls: Record<string, string> = {
  ...
  destructive: "border-brand-destructive/40 bg-brand-destructive/10 text-brand-destructive",
          ...
          {classes.map((c) => (
            <motion.button
              ...
              className={`px-3 py-1.5 text-[12px] font-mono rounded-lg border cursor-pointer outline-none transition-all duration-500 ${
                selected === c.name ? colorCls[c.color] : "border-brand-border bg-brand-surface/30 text-brand-text"
              }`}
            >
              {c.name}
            </motion.button>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[11px] text-brand-text-muted">MRO:</span>
              {mro.map((name, i) => (
                <span key={name} className="flex items-center gap-1">
                  <span
                    className={`text-[11px] font-mono px-2 py-0.5 rounded border ${
                      name === selected ? colorCls[cls.color] : "border-brand-border bg-brand-surface/30 text-brand-text-muted"
                    }`}
                  >
                    {name}
                  </span>
                  {i < mro.length - 1 && (
                    <span className="text-brand-text-subtle text-[10px]">→</span>
                  )}
                </span>
              ))}
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className={`border rounded-lg p-3 ${colorCls[cls.color]}`}>
                ...
                <div className="text-[10px] font-semibold text-brand-text-muted uppercase tracking-wide mb-1.5">
                  Defined here
                </div>
                <div className="space-y-1">
                  {cls.methods.map((m) => (
                    <div key={m} className="text-[11px] font-mono">
                      {m}
                    </div>
                  ))}
                </div>
                {inherited.length > 0 && (
                  <>
                    <div className="text-[10px] font-semibold text-brand-text-muted uppercase tracking-wide mt-3 mb-1.5">
                      Inherited
                    </div>
                    <div className="space-y-1">
                      {inherited.map((m) => (
                        <div key={m} className="text-[11px] font-mono opacity-70">
                          {m}
                        </div>
                      ))}
                    </div>
              ...
              <div className="border border-brand-border rounded-lg p-3 bg-brand-surface/10">
                <div className="text-[11px] font-semibold text-brand-text mb-2">Instance attributes</div>
                <div className="space-y-1">
                  {cls.attrs.map((a) => (
                    <div key={a} className="text-[11px] font-mono text-brand-text-muted">
                      {a}
                    </div>
                  ))}
                </div>
                <div className="text-[10px] text-brand-text-muted mt-3 pt-2 border-t border-brand-border">
                  {cls.parent
