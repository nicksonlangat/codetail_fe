...
const STEPS: StepDef[] = [
  ...
  },
  {
    ...
    title: "DNS Lookup: hostname to IP",
  ...
  },
  {
    ...
    title: "Response: Database to Server",
  ...
  },
  {
    ...
    title: "Response: Server to Load Balancer",
  ...
  },
  {
    ...
    title: "Response: Network to Browser",
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col items-center gap-1 p-2.5 rounded-xl border transition-all duration-300 min-w-0 flex-1 max-w-[100px] ${
        isActive
          ? "border-brand-primary bg-brand-primary/10 shadow-md shadow-brand-primary/20"
          : isPassed
    ...
    >
      ...
      <span className={`text-[10px] font-semibold text-center leading-tight ${isActive ? "text-brand-primary" : "text-brand-text"}`}>
        {node.label}
      </span>
      <span className="text-[8px] text-brand-text-subtle text-center">{node.sublabel}</span>
    </motion.div>
  );
}
...
function EdgeArrow({ isActive, isPassed, dir }: { isActive: boolean; isPassed: boolean; dir: Dir }) {
  ...
  return (
    <div className="flex items-center flex-1 min-w-[12px] max-w-[60px]">
      <div className="flex-1 h-[2px] bg-brand-border relative overflow-hidden rounded-full">
        {highlighted && <div className="absolute inset-0 bg-brand-primary transition-colors duration-300" />}
        {isActive && (
          <motion.div
      ...
      <div
        className={`w-0 h-0 border-t-[4px] border-b-[4px] border-l-[6px] border-transparent shrink-0 transition-colors duration-300 ${
          highlighted ? "border-l-brand-primary" : "border-l-brand-border"
        }`}
      />
    </div>
  );
...
const PHASE_COLORS: Record<Phase, string> = {
  start: "bg-brand-surface text-brand-text-muted",
  request: "bg-brand-primary/10 text-brand-primary",
  response: "bg-brand-success/10 text-brand-success",
      ...
      <div className="flex items-center gap-1 overflow-x-auto p-2 -m-2">
        {NODES.map((node, i) => (
          <div key={node.id} className="flex items-center flex-1 min-w-0">
            <NodeBox node={node} isActive={current.node === node.id} isPassed={passedNodes.has(node.id)} />
            {i < EDGES.length && (
              <EdgeArrow
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <h3 className="text-[13px] font-semibold text-brand-text leading-snug">{current.title}</h3>
            <span className="text-[10px] font-mono text-brand-primary bg-brand-primary/10 px-2 py-0.5 rounded shrink-0">
              {current.latency}
            </span>
          </div>
          <p className="text-[12px] text-brand-text-muted leading-relaxed">{current.body}</p>
        </motion.div>
      </AnimatePresence>
        ...
        <div className="flex-1 flex gap-0.5">
          {STEPS.map((_, i) => (
            <button
              ...
              className={`flex-1 h-1 rounded-full transition-all duration-300 cursor-pointer outline-none ${
                i <= step ? "bg-brand-primary" : "bg-brand-border"
              }`}
            />
          ))}
        </div>
        <span className="text-[10px] text-brand-text-muted tabular-nums shrink-0">
          {step + 1}/{STEPS.length}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <motion.button
            ...
            onClick={prev}
            disabled={step === 0}
            className="p-1.5 rounded-lg border border-brand-border hover:bg-brand-surface transition-all duration-500 cursor-pointer outline-none disabled:opacity-40 disabled:cursor-not-allowed"
          >
