...
> = {
  "chart-2": {
    tab: "hover:text-brand-chart-2",
    activeTab: "bg-brand-chart-2/10 text-brand-chart-2 border-brand-chart-2/30",
    border: "border-brand-chart-2/20",
    badge: "bg-brand-chart-2/10 text-brand-chart-2",
  },
  primary: {
    tab: "hover:text-brand-primary",
    activeTab: "bg-brand-primary/10 text-brand-primary border-brand-primary/30",
    border: "border-brand-primary/20",
    badge: "bg-brand-primary/10 text-brand-primary",
  },
  "chart-4": {
    tab: "hover:text-brand-chart-4",
    activeTab: "bg-brand-chart-4/10 text-brand-chart-4 border-brand-chart-4/30",
    border: "border-brand-chart-4/20",
    badge: "bg-brand-chart-4/10 text-brand-chart-4",
  },
};
  ...
  return (
    <div className="bg-white border border-brand-border rounded-xl overflow-hidden mb-8 not-prose">
      <div className="flex items-center justify-between px-4 py-3 border-b border-brand-border bg-brand-surface/30">
        ...
        <span className={`text-[9px] font-medium px-2 py-0.5 rounded-full ${accent.badge}`}>{current.db}</span>
      </div>
        ...
        {(Object.keys(MODES) as Mode[]).map((m) => {
          ...
          return (
            <button
              ...
              className={`flex-1 py-2.5 text-[11px] font-medium cursor-pointer outline-none transition-all duration-500 border-b-2 ${
                mode === m ? `${a.activeTab} border-b-current` : "text-brand-text-muted border-transparent hover:bg-brand-surface/30"
              }`}
            >
              {MODES[m].label}
            </button>
      ...
      <div className="p-4 space-y-3">
          ...
          {(["schema", "query"] as const).map((v) => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-[10px] font-medium cursor-pointer outline-none transition-all duration-500 ${
                view === v ? "bg-brand-surface text-brand-text" : "text-brand-text-muted hover:text-brand-text"
              }`}
            >
        ...
        <AnimatePresence mode="wait">
          <motion.pre
            ...
            exit={{ opacity: 0, y: -4 }}
            transition={SPRING}
            className={`text-[10px] font-mono bg-brand-surface rounded-xl p-4 overflow-x-auto whitespace-pre leading-relaxed border ${accent.border}`}
          >
          ...
          {current.traits.map(({ icon, label, positive }) => (
            <div
              ...
              className={`flex items-start gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] ${
                positive ? "bg-brand-primary/5 text-brand-text/80" : "bg-brand-surface text-brand-text-muted"
              }`}
            >
              <span className={positive ? "text-brand-primary font-bold" : "text-brand-text-subtle font-bold"}>{icon}</span>
              <span>{label}</span>
            </div>
          ))}
        </div>
