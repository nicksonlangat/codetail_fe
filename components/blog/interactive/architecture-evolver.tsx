        ...
        {(Object.keys(STAGES) as Stage[]).map((s) => (
          <button
            ...
            className={`flex-1 py-2.5 text-[11px] font-semibold cursor-pointer outline-none transition-all duration-500 border-b-2 ${
              stage === s ? "text-brand-primary border-brand-primary bg-brand-surface/30" : "text-brand-text-muted border-transparent hover:bg-brand-surface/20"
            }`}
          >
            {STAGES[s].label}
          </button>
      ...
      <div className="p-4">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={SPRING}
          >
            <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle mb-3">{STAGES[stage].subtitle}</p>

            {stage === "monolith" && (
              <div className="flex flex-col items-center gap-2 mb-5">
                <div className="w-full max-w-xs border-2 border-brand-primary/30 rounded-xl p-3 bg-brand-primary/5">
                  <p className="text-[7px] uppercase tracking-wider text-brand-primary/50 mb-2 text-center">Single Process</p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {MODULES.map((m) => (
                      <div key={m} className="bg-brand-primary/10 border border-brand-primary/20 rounded-lg py-2 text-center">
                        <span className="text-[9px] font-mono text-brand-primary">{m}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="w-px h-4 bg-brand-border" />
                <div className="border border-brand-border rounded-lg px-6 py-1.5 bg-brand-surface/40">
                  <span className="text-[9px] font-mono text-brand-text-muted">Single Database</span>
                </div>
              </div>
            )}
            ...
            {stage === "modular" && (
              <div className="flex flex-col items-center gap-2 mb-5">
                <div className="w-full max-w-xs border-2 border-brand-primary/30 rounded-xl p-3 bg-brand-primary/5">
                  <p className="text-[7px] uppercase tracking-wider text-brand-primary/50 mb-2 text-center">
                    Single Process, Enforced Module Boundaries
                  </p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {MODULES.map((m) => (
                      <div key={m} className="bg-white border-2 border-dashed border-brand-primary/40 rounded-lg py-2 text-center">
                        <span className="text-[9px] font-mono text-brand-primary">{m}</span>
                        <div className="text-[7px] text-brand-text-muted mt-0.5">own schema</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="w-px h-4 bg-brand-border" />
                <div className="border border-brand-border rounded-lg px-4 py-1.5 bg-brand-surface/40">
                  <span className="text-[9px] font-mono text-brand-text-muted">Shared DB, schema per module</span>
                </div>
              </div>
            )}
              ...
              <div className="mb-5">
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {MODULES.map((m, i) => (
                    ...
                    >
                      <div className="w-full border border-brand-primary/30 rounded-xl p-2 bg-brand-primary/5 text-center">
                        <span className="text-[9px] font-mono text-brand-primary font-semibold">{m}</span>
                        <div className="text-[7px] text-brand-text-muted mt-0.5">service</div>
                      </div>
                      <div className="w-px h-3 bg-brand-border" />
                      <div className="border border-brand-border rounded px-2 py-0.5 bg-brand-surface/40">
                        <span className="text-[7px] font-mono text-brand-text-muted">DB</span>
                      </div>
                    </motion.div>
                  ))}
                </div>
                <p className="text-[9px] text-brand-text-muted text-center mt-3">
                  Services communicate via HTTP/gRPC or message queue, no shared memory
                </p>
              </div>
            )}

            <div className="space-y-2 border-t border-brand-border/50 pt-4">
              {PROP_ROWS.map(({ key, label }) => (
                <div key={key} className="flex gap-3 text-[11px]">
                  <span className="text-brand-text-muted w-28 shrink-0 leading-relaxed">{label}</span>
                  <span className="text-brand-text/80 leading-relaxed">{STAGES[stage][key]}</span>
                </div>
              ))}
            </div>
          </motion.div>
