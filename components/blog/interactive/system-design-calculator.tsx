  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between">
        ...
        <span className="text-[11px] font-mono font-semibold text-brand-text">{display}</span>
      </div>
      <input
        type="range"
        min={min}
...
function Metric({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="flex items-center justify-between px-3 py-2 rounded-xl border border-brand-border bg-brand-surface/40">
      <div className="space-y-0.5">
        <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">{label}</p>
        <p className="text-[10px] text-brand-text-muted">{sub}</p>
      </div>
      <span className="font-mono text-[15px] font-bold text-brand-primary ml-4 shrink-0">{value}</span>
    </div>
  );
}
...
export function SystemDesignCalculator() {
  ...
  const scale = m.peakRps < 500 ? "Startup" : m.peakRps < 5000 ? "Growth" : m.peakRps < 50000 ? "Scale" : "Hyperscale";
  ...
  return (
    <div className="not-prose bg-white border border-brand-border rounded-xl overflow-hidden mb-6">
      <div className="px-4 py-2.5 border-b border-brand-border bg-brand-surface/30 flex items-center justify-between">
        <span className="text-[11px] font-semibold text-brand-text">URL shortener capacity estimator</span>
        <span className="text-[9px] font-mono px-2 py-0.5 rounded border border-brand-primary/30 bg-brand-primary/10 text-brand-primary">
          {scale}
        </span>
      </div>
      <div className="grid sm:grid-cols-2 gap-5 p-4">
        <div className="space-y-4">
          <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">Assumptions</p>
          <Slider label="Daily redirects" value={dailyM} min={1} max={1000} step={1} display={dailyM >= 1000 ? "1B" : `${dailyM}M`} onChange={setDailyM} />
          <Slider label="Read / write ratio" value={rwRatio} min={10} max={1000} step={10} display={`${rwRatio}:1`} onChange={setRwRatio} />
          <Slider label="Cache hit rate" value={cacheHit} min={50} max={99} step={1} display={`${cacheHit}%`} onChange={setCacheHit} />
          <Slider label="Data retention" value={retention} min={1} max={10} step={1} display={`${retention} yr`} onChange={setRetention} />
          <Slider label="Avg record size" value={avgBytes} min={50} max={500} step={10} display={`${avgBytes} B`} onChange={setAvgBytes} />
        </div>
        <div className="space-y-2">
          <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">Infrastructure</p>
