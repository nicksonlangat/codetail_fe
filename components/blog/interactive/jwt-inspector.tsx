...
const PART_COLORS: Record<Part, { text: string; bg: string; border: string; label: string }> = {
  header: { text: "text-brand-chart-2", bg: "bg-brand-chart-2/10", border: "border-brand-chart-2/30", label: "Header" },
  payload: { text: "text-brand-primary", bg: "bg-brand-primary/10", border: "border-brand-primary/30", label: "Payload" },
  signature: { text: "text-brand-chart-4", bg: "bg-brand-chart-4/10", border: "border-brand-chart-4/30", label: "Signature" },
};
...
export function JWTInspector() {
  ...
  return (
    <div className="bg-white border border-brand-border rounded-xl overflow-hidden mb-8 not-prose">
      <div className="flex items-center px-4 py-3 border-b border-brand-border bg-brand-surface/30">
      ...
      <div className="p-4 border-b border-brand-border">
        ...
        <div className="flex flex-wrap gap-0 font-mono text-[10px] leading-relaxed bg-brand-surface/30 rounded-xl p-3 border border-brand-border/50">
          {(["header", "payload", "signature"] as Part[]).map((part, i) => (
            <span key={part}>
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                transition={SPRING}
                className={`cursor-pointer outline-none rounded px-0.5 transition-all duration-500 break-all ${
                  active === part ? `${PART_COLORS[part].text} ${PART_COLORS[part].bg} font-bold` : "text-brand-text-muted hover:text-brand-text"
                }`}
              >
                {RAW[part]}
              </motion.button>
              {i < 2 && <span className="text-brand-text-subtle">.</span>}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-2">
          {(["header", "payload", "signature"] as Part[]).map((part) => (
            <button
              key={part}
              onClick={() => toggle(part)}
              className={`flex items-center gap-1.5 cursor-pointer outline-none text-[9px] font-medium transition-all duration-500 ${
                active === part ? PART_COLORS[part].text : "text-brand-text-muted hover:text-brand-text"
              }`}
            >
              <span className={`w-2 h-2 rounded-full ${active === part ? PART_COLORS[part].text.replace("text-", "bg-") : "bg-brand-text-subtle"}`} />
      ...
      <AnimatePresence>
        {active && (
          ...
          >
            <div className={`p-4 border-b border-brand-border ${PART_COLORS[active].bg}`}>
                ...
                <div className="space-y-2">
                  {Object.entries(DECODED_HEADER).map(([k, v]) => (
                    <div key={k} className="flex gap-4 text-[11px]">
                      <span className="font-mono font-semibold text-brand-text/80 w-8 shrink-0">{k}</span>
                      <span className="font-mono text-brand-primary">{v}</span>
                      {k === "alg" && <span className="text-brand-text-muted">{ALG_DOCS[v as string] ?? ""}</span>}
                      {k === "typ" && <span className="text-brand-text-muted">Token type, always &quot;JWT&quot;</span>}
                    </div>
                  ))}
                </div>
              )}
                    ...
                    return (
                      <div key={k} className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-semibold text-[11px] text-brand-text/80 w-10 shrink-0">{k}</span>
                          <span className="font-mono text-[10px] text-brand-primary">{fmt(v, k)}</span>
                          {doc && (
                            <span
                              className={`text-[8px] px-1.5 py-0.5 rounded font-mono border ${
                                doc.standard ? "text-brand-primary bg-brand-primary/10 border-brand-primary/20" : "text-brand-text-muted bg-brand-surface border-brand-border"
                              }`}
                            >
                        ...
                        {doc && (
                          <p className="text-[10px] text-brand-text-muted ml-12">
                            {doc.name}: {doc.desc}
                          </p>
                        )}
                      </div>
              ...
              {active === "signature" && (
                <div className="space-y-3 text-[11px]">
                  <div className="font-mono text-[10px] bg-brand-surface/50 rounded-xl p-3 leading-relaxed text-brand-text-muted">
                  ...
                  <div className="space-y-1.5">
                    {[
                      ...
                      <p key={label}>
                        <span className="font-semibold text-brand-text/80">{label}: </span>
                        <span className="text-brand-text-muted">{val}</span>
                      </p>
                    ))}
                  </div>
                </div>
      ...
      {!active && (
        <div className="px-4 py-3">
          <p className="text-[9px] text-brand-text-subtle text-center">Click any segment above to decode it</p>
        </div>
      )}
    </div>
  );
