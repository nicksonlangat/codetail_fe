...
> = {
  rest: {
    label: "REST",
    badge: "text-brand-chart-2",
    badgeBg: "bg-brand-chart-2/10 border-brand-chart-2/20",
    roundTrips: 3,
    payloadSize: "~8 KB",
    payloadBytes: 8192,
    requestLabel: "3 HTTP requests required",
    responseLabel: "Response, 5 fields needed, 10+ returned",
    request: `GET /api/v1/posts/42
...
service PostService {
  rpc GetPostCard(GetPostCardRequest)
    ...
    response: `Binary (Protocol Buffers):
...
export function APIComparator() {
  ...
  return (
    <div className="bg-white border border-brand-border rounded-xl overflow-hidden mb-8 not-prose">
        ...
        {(Object.keys(MODES) as API[]).map((a) => (
          <button
            ...
            className={`flex-1 py-2.5 text-[11px] font-semibold cursor-pointer outline-none transition-all duration-500 border-b-2 ${
              api === a ? `${MODES[a].badge} border-current bg-brand-surface/30` : "text-brand-text-muted border-transparent hover:bg-brand-surface/20"
            }`}
          >
            {MODES[a].label}
          </button>
      ...
      <div className="flex items-center gap-4 px-4 py-2.5 border-b border-brand-border/50 bg-brand-surface/20">
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] text-brand-text-muted">Round trips</span>
          <span className={`text-[11px] font-mono font-bold ${mode.roundTrips === 1 ? "text-brand-primary" : "text-brand-warning"}`}>
            {mode.roundTrips}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] text-brand-text-muted">Payload</span>
          <span className={`text-[11px] font-mono font-bold ${mode.payloadBytes < 500 ? "text-brand-primary" : "text-brand-warning"}`}>
            {mode.payloadSize}
          </span>
        </div>
        <div className="flex-1 ml-2">
          <div className="h-1.5 bg-brand-surface rounded-full overflow-hidden">
            <motion.div
              className={`h-full rounded-full ${mode.payloadBytes < 500 ? "bg-brand-primary" : "bg-brand-warning"}`}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, y: -6 }}
          transition={SPRING}
          className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-brand-border/50"
        >
          <div className="p-4 space-y-2">
            <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">{mode.requestLabel}</p>
            <pre className={`text-[9px] font-mono rounded-xl p-3 overflow-x-auto whitespace-pre leading-relaxed border ${mode.badgeBg}`}>
              {mode.request}
            </pre>
          </div>
          <div className="p-4 space-y-2">
            <p className="text-[9px] uppercase tracking-wider text-brand-text-subtle">{mode.responseLabel}</p>
            <pre className="text-[9px] font-mono bg-brand-surface rounded-xl p-3 overflow-x-auto whitespace-pre leading-relaxed">
              {mode.response}
            </pre>
          </div>
        </motion.div>
