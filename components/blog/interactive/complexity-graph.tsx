      ...
      <div className="flex flex-wrap gap-5 mt-3">
        {lines.map((line) => (
          <div key={line.label} className="flex items-start gap-2">
            <div
              className="w-3 h-0.5 mt-[7px] shrink-0 rounded-full"
              style={{ backgroundColor: line.color }}
            />
            <div>
              <span className="text-[11px] font-mono font-medium block" style={{ color: line.color }}>
                {line.label}
              </span>
              <p className="text-[11px] text-brand-text-muted">{line.description}</p>
            </div>
          </div>
        ))}
      </div>
