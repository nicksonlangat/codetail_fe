...
import {
  modernPythonArticles,
  getArticleBySlug,
  getAdjacentArticles,
} from "@/content/modern-python/registry";
import { loadArticle } from "@/content/modern-python/article-loader";

export function generateStaticParams() {
  return modernPythonArticles.map((article) => ({ slug: article.slug }));
}
  ...
  return (
    <>
      <BlogLayout toc={toc} sidebar={sidebar}>
        <ArticleShell
          article={article}
          totalArticles={modernPythonArticles.length}
          basePath="/blog/modern-python"
          backLabel="All Modern Python articles"
          prev={prev}
          next={next}
        >
          {ArticleComponent ? (
