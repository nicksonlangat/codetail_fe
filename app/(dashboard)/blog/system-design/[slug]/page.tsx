...
import { systemDesignArticles, getArticleBySlug, getAdjacentArticles } from "@/content/system-design/registry";
import { loadArticle } from "@/content/system-design/article-loader";

export function generateStaticParams() {
  return systemDesignArticles.map((article) => ({ slug: article.slug }));
}
  ...
  return (
    <>
      <BlogLayout toc={toc} sidebar={sidebar}>
        <ArticleShell
          article={article}
          totalArticles={systemDesignArticles.length}
          basePath="/blog/system-design"
          backLabel="All System Design articles"
          prev={prev}
          next={next}
        >
          {ArticleComponent ? (
