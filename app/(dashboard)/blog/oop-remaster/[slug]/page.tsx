...
import {
  ...
  getArticleBySlug,
  getAdjacentArticles,
} from "@/content/oop-remaster/registry";
import { loadArticle } from "@/content/oop-remaster/article-loader";

export function generateStaticParams() {
  return oopRemasterArticles.map((article) => ({ slug: article.slug }));
}
  ...
  return (
    <>
      <BlogLayout toc={toc} sidebar={sidebar}>
        <ArticleShell
          article={article}
          totalArticles={oopRemasterArticles.length}
          basePath="/blog/oop-remaster"
          backLabel="All OOP Remaster articles"
          prev={prev}
          next={next}
        >
          {ArticleComponent ? (
