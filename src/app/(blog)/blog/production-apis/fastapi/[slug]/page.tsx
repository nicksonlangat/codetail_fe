...
import { fastapiArticles, getArticleBySlug, getAdjacentArticles } from "@/content/production-apis/fastapi/registry";
...
import { loadArticle } from "@/content/production-apis/fastapi/article-loader";

export function generateStaticParams() {
  return fastapiArticles.map((article) => ({ slug: article.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!article) return {};
  return {
    title: `${article.title} | Production-Ready APIs | Codetail`,
