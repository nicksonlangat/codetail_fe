...
import { conceptsArticles } from "@/content/concepts/registry";

const spring = { type: "spring" as const, stiffness: 300, damping: 30 };

export default function ConceptsSeriesPage() {
  const totalMinutes = conceptsArticles.reduce((sum, a) => sum + a.estimatedMinutes, 0);
    ...
    <main className="max-w-4xl mx-auto px-6 lg:px-8 py-8 space-y-10">
      ...
      >
        ...
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">
          Concepts Every{" "}
          <span className="text-primary">Backend Engineer</span> Should Know
        </h1>
        <p className="text-[14px] text-muted-foreground max-w-2xl leading-relaxed">
          Standalone deep-dives on specific backend and distributed systems concepts.
          Practical, precise, and production-focused. Each article stands on its own.
        </p>
        <div className="flex items-center gap-4 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <BookOpen className="w-3.5 h-3.5" />
            {conceptsArticles.length} articles
          </span>
      ...
      >
        {[
          { icon: "🔁", label: "Idempotency" },
          { icon: "⚡", label: "Race Conditions" },
          { icon: "🔒", label: "Locking Strategies" },
          { icon: "📬", label: "At-least-once Delivery" },
          { icon: "⚛️", label: "Atomic Operations" },
          { icon: "🗄️", label: "Isolation Levels" },
        ].map(({ icon, label }) => (
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {conceptsArticles.map((article, index) => (
            <motion.div
              key={article.slug}
            ...
            >
              <Link
                href={`/blog/concepts/${article.slug}`}
